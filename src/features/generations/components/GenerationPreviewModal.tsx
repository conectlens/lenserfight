        ...
        `}>
                ...
                {media.media_kind === 'audio' && (
                    <div className="w-full p-10 flex flex-col items-center">
                        <div className="w-24 h-24 rounded-full bg-gray-200 dark:bg-gray-800 flex items-center justify-center mb-6">
                            <svg className="w-10 h-10 text-gray-500" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 19V6l12-3v13M9 10l12-3" /></svg>
                        </div>
            ...
            `}>
                <div className="flex items-center justify-between mb-6 flex-shrink-0">
                    ...
                    <button onClick={onClose} className="p-2 rounded-full transition-colors hover:bg-gray-100 dark:hover:bg-gray-800">
                        <X size={20} />
                    </button>
                </div>
                ...
                <div className="space-y-6 flex-1 md:overflow-y-auto generations-scroll">
                    <div>
                        <span className="text-xs font-bold uppercase tracking-wider text-gray-400 dark:text-gray-500">Model</span>
                        <p className="font-medium mt-1 text-sm">{modelLabel}</p>
                    </div>

                    <div>
                        <span className="text-xs font-bold uppercase tracking-wider text-gray-400 dark:text-gray-500">Created</span>
                        <div className="flex items-center gap-2 mt-1">
                            <Calendar size={14} className="text-gray-500 dark:text-gray-400" />
                            <p className="font-medium text-sm">{new Date(generation.created_at).toLocaleDateString()}</p>
                        </div>
                    </div>

                    {generation.input_text && media.media_kind !== 'text' && (
                        <div>
                            <span className="text-xs font-bold uppercase tracking-wider text-gray-400 dark:text-gray-500">Input</span>
