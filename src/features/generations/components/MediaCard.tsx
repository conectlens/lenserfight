...
import { MediaLibraryItem } from '../../../types/generation.types';
...
export const MediaCard: React.FC<MediaCardProps> = ({ media, onClick, textPreview }) => {
      ...
      >
        ...
        {!imgError ? (
            <img 
                ...
                onError={() => { setIsLoaded(true); setImgError(true); }}
                loading="lazy"
            />
        ) : (
  ...
  if (media.media_kind === 'video') {
    return (
      <div className="relative w-full h-full group cursor-pointer overflow-hidden rounded-xl bg-gray-900" onClick={handleMediaClick}>
        <video 
            src={media.url} 
  ...
  if (media.media_kind === 'audio') {
      return (
        <div className="relative w-full h-full group cursor-pointer overflow-hidden rounded-xl bg-gradient-to-br from-gray-800 to-gray-900 p-6 flex flex-col justify-center items-center text-white min-h-[140px]" onClick={handleMediaClick}>
            ...
            <p className="text-xs font-medium text-gray-300 truncate max-w-full px-2">{media.file_name}</p>
            {media.duration_seconds && <span className="text-[10px] text-gray-500 mt-1">{Math.floor(media.duration_seconds)}s</span>}
        </div>
      );
  }
