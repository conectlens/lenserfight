    ...
    <AuthCard title="Sign In" subtitle="Welcome back, Lenser" backButton={backButton}>
      ...
      <form onSubmit={handleSubmit} className="space-y-5" noValidate>
        ...
        <div className="flex items-center justify-between text-sm pt-1">
          ...
          <Link to="/forgot-password" className="font-semibold text-primary-700 hover:text-primary-800 hover:underline">
            Forgot password?
          </Link>
        </div>
...
<div className="grid grid-cols-2 gap-3 mt-4">
  ...
  <button
    ...
    className="flex items-center justify-center py-2.5 border border-gray-200 rounded-xl bg-white shadow-sm hover:bg-gray-50 transition"
  >
  ...
  <button
    ...
    onClick={async () => {
      ...
      try {
        ...
        setApiError(err.message || 'GitHub sign-in failed');
      } finally {
        setLoading(false);
      }
    }}
  ...
  >
    <svg className="w-5 h-5 text-gray-800" fill="currentColor" viewBox="0 0 24 24">
      ...
      <div className="mt-8 text-center text-sm text-gray-500 font-medium">
        ...
        <Link 
          to="/register" 
          className="ml-1.5 text-gray-900 hover:text-primary-700 font-bold hover:underline transition-colors"
        >
          Join ConnectLens
        </Link>
      </div>
