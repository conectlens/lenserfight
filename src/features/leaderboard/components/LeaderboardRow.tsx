...
export const LeaderboardRow: React.FC<LeaderboardRowProps> = ({ entry, isCurrentUser }) => {
  ...
  const getRankBadge = () => {
    ...
    return <span className="text-xl font-bold text-gray-400 dark:text-gray-500 w-10 text-center">#{rank}</span>;
  };

  const getContainerStyles = () => {
      let base = "flex items-center gap-4 md:gap-6 p-4 rounded-2xl transition-all cursor-pointer group border";
      
      // Top 3 Visuals
      if (isGold) {
          base += " bg-gradient-to-r from-yellow-50/80 via-white to-white dark:from-yellow-900/10 dark:via-gray-800 dark:to-gray-800 border-yellow-200 dark:border-yellow-700/50 shadow-sm";
      } else if (isSilver) {
          base += " bg-gradient-to-r from-slate-50/80 via-white to-white dark:from-slate-800/30 dark:via-gray-800 dark:to-gray-800 border-slate-200 dark:border-slate-700/50 shadow-sm";
      } else if (isBronze) {
          base += " bg-gradient-to-r from-orange-50/80 via-white to-white dark:from-orange-900/10 dark:via-gray-800 dark:to-gray-800 border-orange-200 dark:border-orange-800/50 shadow-sm";
      } else if (isCurrentUser) {
          // Standard User Highlight if not in Top 3
          base += " bg-primary/5 border-primary/30 dark:bg-primary/5 shadow-md";
      } else {
          // Default
          base += " bg-white dark:bg-gray-800 border-transparent hover:border-gray-200 dark:hover:border-gray-700 hover:shadow-md";
      }
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center gap-4 flex-1 min-w-0">
        ...
        <div className="flex flex-col overflow-hidden">
            <div className="flex items-center gap-2">
                <span className={`font-bold truncate text-lg ${isCurrentUser ? 'text-primary-900 dark:text-primary-400' : 'text-gray-900 dark:text-white'}`}>
                    {displayName}
                </span>
      ...
      <div className="hidden md:flex items-center gap-8">
         ...
         <div className={`px-3 py-1 rounded-full text-xs font-bold ${isGold ? 'bg-yellow-100 text-yellow-800 dark:bg-yellow-900/30 dark:text-yellow-200' : 'bg-gray-100 dark:bg-gray-700 text-gray-600 dark:text-gray-300'}`}>
             Level {level}
         </div>
      </div>
      ...
      <div className="flex flex-col items-end min-w-[80px]">
          ...
          <div className="md:hidden flex items-center gap-2 mt-1">
             <span className={`text-xs font-medium px-2 rounded-full ${isGold ? 'text-yellow-800 bg-yellow-100 dark:bg-yellow-900/30 dark:text-yellow-200' : 'text-gray-500 bg-gray-100 dark:bg-gray-700'}`}>Lvl {level}</span>
          </div>
      </div>
