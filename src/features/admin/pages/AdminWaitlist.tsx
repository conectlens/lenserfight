...
export const AdminWaitlist: React.FC = () => {
  ...
  const columns = [
    { header: 'User', render: (u: any) => <div className="flex items-center gap-2"><Avatar src={u.avatar_url} size="sm" /> <span>{u.display_name}</span></div> },
    { header: 'Handle', accessor: 'handle' as const },
    { header: 'Joined Date', render: (u: any) => new Date(u.created_at).toLocaleDateString() },
    { header: 'Status', render: () => <span className="text-green-600 font-bold text-xs">WAITING</span> },
  ];
