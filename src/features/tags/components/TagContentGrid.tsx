                   ...
                   <div key={item.id} onClick={() => navigate(`/threads/${item.id}`)} className="cursor-pointer group h-full">
                       <Card className="h-full flex flex-col hover:shadow-lg transition-all border-gray-200 hover:border-primary/40 p-6">
                           <div className="flex items-center gap-3 mb-4">
                               <Avatar src={thread.author.avatarUrl} size="sm" className="!w-8 !h-8 ring-2 ring-white" />
                               <span className="text-xs font-semibold text-gray-700 truncate">{thread.author.displayName}</span>
                               <span className="text-xs text-gray-400 ml-auto">{timeAgo(thread.createdAt)}</span>
                           </div>
                           <h3 className="font-bold text-lg text-gray-900 mb-3 line-clamp-2 leading-tight group-hover:text-primary-800 transition-colors">
                               {thread.title}
                           </h3>
                           ...
                           <div className="flex items-center gap-4 pt-3 border-t border-gray-50 text-xs font-medium text-gray-500 mt-auto">
                               <div className="flex items-center gap-1.5 hover:text-primary-700 transition-colors">
