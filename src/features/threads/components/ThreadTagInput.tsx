...
export const ThreadTagInput: React.FC<ThreadTagInputProps> = ({ tags, onChange }) => {
  ...
  return (
    <div className="flex flex-col gap-2">
      ...
      <div
        className={`flex flex-wrap items-center gap-2 px-3 py-2.5 bg-gray-50 dark:bg-gray-800 border ${isLoading ? 'border-primary' : 'border-gray-200 dark:border-gray-700'} rounded-xl focus-within:ring-2 focus-within:ring-primary/50 focus-within:border-primary transition-all`}
      >
        {tags.map((tag) => (
          <span
            key={tag.slug}
