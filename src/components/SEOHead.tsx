...
type SEOType = 'home' | 'prompt' | 'thread' | 'profile' | 'tag' | 'tag-cloud' | 'prompts-list' | 'default';
...
export const SEOHead: React.FC<SEOHeadProps> = ({ type, data, overrideTitle }) => {
  const meta: SEOMetadata = useMemo(() => {
    ...
    switch (type) {
      case 'home': return seoService.getHomeMeta();
      case 'prompt': return seoService.getPromptMeta(data);
      case 'thread': return seoService.getThreadMeta(data);
      case 'profile': 
      ...
      case 'tag': return seoService.getTagMeta(data);
      case 'tag-cloud': return seoService.getTagCloudMeta();
      case 'prompts-list': return seoService.getPromptsListMeta();
      default: return seoService.getHomeMeta();
    }
