...
export class MockReactionRepository implements ReactionRepositoryPort {
  ...
  async getUserReaction(
    ...
    return this.getStore().filter(
      (r) => r.target_type === targetType && r.target_id === targetId && r.lenser_id === lenserId
    )
  }
  ...
  async getBatchUserReactions(
    ...
    return this.getStore().filter(
      (r) =>
        r.target_type === targetType && r.lenser_id === lenserId && targetIds.includes(r.target_id)
    )
  }
