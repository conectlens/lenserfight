...
export class MockAdminRepository implements AdminRepositoryPort {
  ...
  async getUsers(offset: number, limit: number, query?: string): Promise<AdminListResponse<AdminUser>> {
    ...
    if (query) {
        users = users.filter(u => u.email.toLowerCase().includes(query.toLowerCase()) || u.display_name.toLowerCase().includes(query.toLowerCase()));
    }
  ...
  async getFeedbacks(offset: number, limit: number, status?: string): Promise<AdminListResponse<Feedback>> {
    await new Promise(r => setTimeout(r, 400));
  ...
  async getWaitlist(offset: number, limit: number): Promise<AdminListResponse<Lenser>> {
    ...
    waitlisted.sort((a: Lenser, b: Lenser) => new Date(b.created_at).getTime() - new Date(a.created_at).getTime());
