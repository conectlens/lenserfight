...
const DEFAULT_DESC = 'Join the LenserFight ecosystem. Discover, battle, and share advanced AI prompts for GPT-5, Gemini, Claude, and Midjourney. Connect with top Lensers and master LLM interactions.';

// Keywords for injection based on context
const AI_KEYWORDS = ['Generative AI', 'LLM', 'Prompt Engineering', 'GPT-5', 'Gemini Ultra', 'Claude 3', 'Lenser AI', 'Midjourney v6', 'Stable Diffusion', 'Sora'];
...
export const seoService = {
  ...
  getPromptMeta: (prompt?: PromptTemplateDetailViewModel | PromptTemplateViewModel | null): SEOMetadata => {
    if (!prompt) return { title: 'Prompt Not Found', description: 'This prompt template could not be found on LenserFight.' };
  ...
  getThreadMeta: (thread?: ThreadDetailViewModel | ThreadFeedItem | null): SEOMetadata => {
    if (!thread) return { title: 'Thread Not Found', description: 'This discussion could not be found on LenserFight.' };
  ...
  getProfileMeta: (lenser?: Lenser | null, stats?: LenserStats | null): SEOMetadata => {
    if (!lenser) return { title: 'Lenser Not Found', description: 'User profile not found on LenserFight.' };

    const role = lenser.headline || 'AI Creator';
    const statText = stats ? `${stats.promptsCount} prompts, ${stats.threadsCount} threads.` : 'Active Lenser community member.';
  ...
  getTagMeta: (tag?: TagUsage | null): SEOMetadata => {
    if (!tag) return { title: 'Explore AI Topics', description: 'Discover trending AI topics and prompts.' };
