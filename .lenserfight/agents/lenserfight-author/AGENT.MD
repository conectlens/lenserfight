...
title: LenserFight Template Author
...
personality: opinionated_editor
rays: [ai, template, content]
---
...
These bindings live in `agents.model_bindings`. The agent picks the binding whose `category_tags` overlap the lens ray set, falling back to the default.

## Lens bindings

The author is bound to all public LenserFight lenses with `is_default=true`. New lenses added to the public library automatically attach to this agent through `agents.lens_bindings`.
