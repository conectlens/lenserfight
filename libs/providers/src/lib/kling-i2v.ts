import type { GenerativeMediaAdapter, AsyncGenerationResponse } from './types';
...
export const klingI2vAdapter: GenerativeMediaAdapter = {
  ...
  async pollTask(apiKey, taskId): Promise<import('./types').GenerativeMediaResult> {
    ...
    if (task.task_status === 'failed') {
      return { status: 'failed', urls: [], mimeType: 'video/mp4' };
    }
    return { status: 'pending', urls: [], mimeType: 'video/mp4' };
  },
};
