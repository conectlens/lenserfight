...
export function WorkflowCronPanel({ workflowId, isOwner }: WorkflowCronPanelProps) {
  const { data: schedules = [], isLoading } = useWorkflowSchedules(workflowId)
  const { mutate: upsertSchedule, isPending: upserting } = useUpsertWorkflowSchedule(workflowId)
  ...
  const handleAdd = () => {
    ...
    upsertSchedule(
      ...
      { onSuccess: () => { setShowForm(false); setCronExpr('') } },
    )
  }
