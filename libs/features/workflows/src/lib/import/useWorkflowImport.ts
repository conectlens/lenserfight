      ...
      try {
        ...
        const imported = await importWorkflow(
          text,
          {
            ...
            /**
             * `getLatestPublishedVersion` is the right source twice over.
             *
             * It costs one round-trip — `fn_get_lens_detail_bootstrap` returns
             * the version and its parameters together — where `getHeadVersion`
             * and `getVersionById` each chain two or three RPCs.
             *
             * More importantly it reads the version the workflow will actually
             * bind to: imported nodes are persisted with `version_id: null`, so
             * execution resolves the lens through this same call. Checking
             * compatibility against an unpublished draft would let a node reuse
             * a lens that cannot supply its parameters at run time.
             */
            getLensParameterLabels: async (lensId) => {
              const version = await lensesService.getLatestPublishedVersion(lensId)
              return (version?.parameters ?? []).map((parameter) => parameter.label)
            },
