...
export const WORKFLOW_NODE_CATALOG: CatalogNodeEntry[] = [
  ...
  },
  {
    ...
    description: 'Register an inbound webhook URL that triggers this workflow.',
    category: 'storage',
    color: 'text-amber-500',
    iconName: 'Webhook',
    inputs: [],
    outputs: [{ name: 'payload', type: 'json', description: 'Incoming webhook payload' }],
    requiredConfig: ['secret'],
    optionalConfig: [],
    exampleConfig: { secret: 'my-secure-webhook-secret-key' },
  ...
  },
  {
    ...
    label: 'Schedule',
    description: 'Configure cron-based workflow scheduling.',
    category: 'storage',
    color: 'text-amber-500',
    iconName: 'Calendar',
    inputs: [],
    outputs: [{ name: 'trigger', type: 'json', description: 'Schedule metadata' }],
    requiredConfig: ['cronExpression'],
    optionalConfig: ['timezone', 'enabled'],
    exampleConfig: { cronExpression: '0 9 * * 1', timezone: 'Europe/Istanbul' },
    acceptsInputTypes: ['void'],
    producesOutputType: 'json',
