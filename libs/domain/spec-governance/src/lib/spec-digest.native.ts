/**
 * Native (Expo/React Native) override of spec-digest.
 * node:crypto is not available on Hermes — uses the pure-JS SHA-256 fallback directly.
 * The pure-JS implementation is identical to the Node.js native output.
 */
...
export function computeExecutionSeal(
  ...
  return sha256(`${specContentHash}|${inputHash}|${outputHash}`)
}
