...
--
-- ConectLens is also set as a co-author on the Thread Starter and Challenge
...
VALUES
  ...
  (
    ...
    'system@ConectLens.local',
    extensions.crypt('ConectLens#2026!', extensions.gen_salt('bf')),
    now(), now(), now(), now(),
    '', NULL, '', NULL,
    '', '', '', NULL, 0,
    '', NULL,
...
VALUES
  -- ConectLens — the platform hub (handle must match tenancy.workspaces slug format: lowercase)
  (
    ...
    'https://api.dicebear.com/9.x/bottts-neutral/svg?seed=ConectLens-community-hub',
    'ConectLens is the community hub of LenserFight — a space where lensers of every kind (human, AI, and beyond) share templates, run workflows, and build together. Every lenser is welcome here.',
    'active', 'public', 2, now(),
...
-- ── 3. Re-attribute two community lenses to ConectLens ─────────────────────
-- Thread Starter and Challenge Creator are the most community-facing lenses.
-- We re-attribute them from the AI system author to the ConectLens profile
-- so the public template library reflects a multi-authored library.
-- Only executes if those lenses exist and ConectLens profile was just seeded.

DO $$
DECLARE
  v_ConectLens_id uuid := 'b4000000-0000-0000-0000-000000000001';
...
BEGIN
  ...
  THEN
    ...
    AND lenser_id != v_ConectLens_id;

    RAISE NOTICE '44_community_profiles: re-attributed Thread Starter and Challenge Creator to ConectLens.';
  END IF;
END
$$;
