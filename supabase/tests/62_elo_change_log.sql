...
VALUES
  ('11111111-bv04-1111-1111-111111111111', 'bv-elo-creator@test.local'),
  ('22222222-bv04-2222-2222-222222222222', 'bv-elo-winner@test.local'),
  ('33333333-bv04-3333-3333-333333333333', 'bv-elo-loser@test.local')
ON CONFLICT (id) DO NOTHING;

INSERT INTO lensers.profiles (id, user_id, handle, display_name, type)
VALUES
  ('11111111-bv04-1111-1111-111111111111',
   '11111111-bv04-1111-1111-111111111111', 'bv_elo_creator', 'BV ELO Creator', 'human'),
  ('22222222-bv04-2222-2222-222222222222',
   '22222222-bv04-2222-2222-222222222222', 'bv_elo_winner', 'BV ELO Winner', 'human'),
  ('33333333-bv04-3333-3333-333333333333',
   '33333333-bv04-3333-3333-333333333333', 'bv_elo_loser', 'BV ELO Loser', 'human')
ON CONFLICT (id) DO NOTHING;

INSERT INTO battles.battles (
  id, creator_lenser_id, title, slug, task_prompt, status, max_contenders
) VALUES (
  'bbbb04bb-bv04-bbbb-bbbb-bbbbbbbbbbbb',
  '11111111-bv04-1111-1111-111111111111',
...
INSERT INTO battles.contenders (
  ...
  ('cccc04cc-bv04-cccc-cccc-aaaaaaaaaaaa',
   'bbbb04bb-bv04-bbbb-bbbb-bbbbbbbbbbbb', 'A',
   'human'::battles.contender_type_enum,
   '22222222-bv04-2222-2222-222222222222', 'A', 'accepted'),
  ('cccc04cc-bv04-cccc-cccc-bbbbbbbbbbbb',
   'bbbb04bb-bv04-bbbb-bbbb-bbbbbbbbbbbb', 'B',
   'human'::battles.contender_type_enum,
   '33333333-bv04-3333-3333-333333333333', 'B', 'accepted')
ON CONFLICT (id) DO NOTHING;

-- Mark the winner.
UPDATE battles.battles
   SET winner_contender_id = 'cccc04cc-bv04-cccc-cccc-aaaaaaaaaaaa',
       finalized_at        = now()
 WHERE id = 'bbbb04bb-bv04-bbbb-bbbb-bbbbbbbbbbbb';

-- ── Drive the ELO compute (service-role context). ──────────────────────────
SELECT public.fn_compute_elo_after_battle(
  'bbbb04bb-bv04-bbbb-bbbb-bbbbbbbbbbbb'::uuid
);

-- ── Test 1: a log row exists for the battle ────────────────────────────────
SELECT is(
  (SELECT count(*)::int FROM reputation.elo_battle_log
    WHERE battle_id = 'bbbb04bb-bv04-bbbb-bbbb-bbbbbbbbbbbb'),
  1,
