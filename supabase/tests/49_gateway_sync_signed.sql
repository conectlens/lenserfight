...
VALUES ('aaaaaaaa-bg01-aaaa-aaaa-aaaaaaaaaaaa', 'bg-owner@test.local')
ON CONFLICT (id) DO NOTHING;

SET LOCAL "request.jwt.claims" TO '{"sub":"aaaaaaaa-bg01-aaaa-aaaa-aaaaaaaaaaaa","role":"authenticated"}';
SET LOCAL ROLE authenticated;

SELECT public.fn_gateway_heartbeat(
  'cccccccc-bg01-cccc-cccc-cccccccccccc'::uuid,
  repeat('A', 44),
  'host-bg',
  'lf-gatewayd/0.1.0'
);
...
VALUES ('cccccccc-bg01-cccc-cccc-cccccccccccc'::uuid, 'noop', '{}'::jsonb);
...
SELECT is(
  (SELECT envelope_sig
     FROM public.fn_gateway_claim_commands_v2(
       'cccccccc-bg01-cccc-cccc-cccccccccccc'::uuid, 5
     )
   LIMIT 1)::text,
  NULL,
...
SELECT lives_ok(
  $$ INSERT INTO agents.gateway_commands (device_id, command_type, payload)
     VALUES ('cccccccc-bg01-cccc-cccc-cccccccccccc'::uuid, 'config_push', '{"k":"v"}'::jsonb) $$,
...
SELECT is(
  (SELECT envelope_sig
     FROM public.fn_gateway_claim_commands_v2(
       'cccccccc-bg01-cccc-cccc-cccccccccccc'::uuid, 50
     )
