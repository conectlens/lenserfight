...
drop policy "page_views_insert_secure" on "analytics"."page_views";

drop policy "p_feedback_insert_public" on "analytics"."product_feedback";

drop policy "Public can read prompt reactions" on "content"."prompt_reactions";

drop policy "prompt_templates_select_public" on "content"."prompt_templates";

drop policy "Public can read prompt translations" on "content"."prompt_translations";

drop policy "Public can read thread reactions" on "content"."thread_reactions";

drop policy "thread_replies_public_select" on "content"."thread_replies";

drop policy "Public can read reply reactions" on "content"."thread_reply_reactions";

drop policy "Public can read thread translations" on "content"."thread_translations";

drop policy "threads_public_select" on "content"."threads";
...
with check (((path IS NOT NULL) AND (length(path) > 0)));
  ...
  create policy "p_feedback_insert_public"
...
using ((EXISTS ( SELECT 1
  ...
  for select
  to anon, authenticated
using ((visibility = 'public'::content.visibility_enum));
