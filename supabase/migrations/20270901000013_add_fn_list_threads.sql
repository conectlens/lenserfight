...
COMMENT ON FUNCTION public.fn_list_threads(uuid, int, text) IS
  'Lists threads owned by the current authenticated user (all visibilities). Used by threadsRepository.getByLenser(includePrivate=true) when the viewer is the profile owner. Supports cursor pagination and optional tag-slug filtering.';
