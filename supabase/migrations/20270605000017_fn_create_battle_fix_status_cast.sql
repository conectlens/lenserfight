-- Follow-up: battles.battles.status is also an enum
-- (battles.battle_status_enum), and the CTE SELECT in the previous version
-- of this function selected it unqualified — "structure of query does not
-- match function result type" against the text-declared RETURNS TABLE
-- column. Cast added.
--
...
BEGIN
  ...
  SELECT
    ins.id, ins.slug, ins.title, ins.task_prompt, ins.status::text, ins.total_vote_count,
