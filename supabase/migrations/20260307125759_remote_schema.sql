...
CREATE OR REPLACE FUNCTION "content"."adjust_reaction_totals"("_target_type" "text", "_target_id" "uuid", "_reaction" "text", "_delta" integer) RETURNS "void"
    LANGUAGE "plpgsql"
    AS $$begin
  -- THREAD
  if _target_type = 'thread' then
    ...
    set reaction_totals =
      coalesce(reaction_totals, '{}'::jsonb)
      || jsonb_build_object(
           _reaction,
           greatest(
             0,
             coalesce((reaction_totals ->> _reaction)::integer, 0) + _delta
           )
         )
    where id = _target_id;

  -- THREAD_REPLY
  elsif _target_type = 'thread_reply' then
    update content.thread_replies
    set reaction_totals =
      coalesce(reaction_totals, '{}'::jsonb)
      || jsonb_build_object(
           _reaction,
  ...
  -- PROMPT_TEMPLATE
  elsif _target_type = 'prompt_template' then
    ...
    -- Unknown target_type: do nothing (safe no-op)
    null;
  end if;
end;$$;


ALTER FUNCTION "content"."adjust_reaction_totals"("_target_type" "text", "_target_id" "uuid", "_reaction" "text", "_delta" integer) OWNER TO "postgres";
...
CREATE OR REPLACE FUNCTION "content"."prevent_protected_prompt_fields"() RETURNS "trigger"
    LANGUAGE "plpgsql"
    AS $$DECLARE
  jwt_role text := current_setting('request.jwt.claim.role', true);
BEGIN
  -- Allow system-level operations to modify protected fields
  IF jwt_role = 'service_role' OR current_user = 'postgres' THEN
    RETURN NEW;
  END IF;

  -- Block manual user modifications
  IF NEW.reaction_totals <> OLD.reaction_totals THEN
    RAISE EXCEPTION 'reaction_totals cannot be modified manually.';
  END IF;

  IF NEW.created_at <> OLD.created_at THEN
    RAISE EXCEPTION 'created_at cannot be modified manually.';
  END IF;

  -- updated_at can only be changed by system triggers
...
ALTER FUNCTION "content"."prevent_protected_prompt_fields"() OWNER TO "postgres";


CREATE OR REPLACE FUNCTION "content"."reactions_after_delete_trigger"() RETURNS "trigger"
    LANGUAGE "plpgsql"
    AS $$begin
  perform content.adjust_reaction_totals(
    OLD.target_type,
    OLD.target_id,
    OLD.reaction::text,
    -1
...
ALTER FUNCTION "content"."reactions_after_delete_trigger"() OWNER TO "postgres";


CREATE OR REPLACE FUNCTION "content"."reactions_after_insert_trigger"() RETURNS "trigger"
    LANGUAGE "plpgsql"
    AS $$begin
  perform content.adjust_reaction_totals(
    NEW.target_type,
...
ALTER FUNCTION "content"."reactions_after_insert_trigger"() OWNER TO "postgres";


CREATE OR REPLACE FUNCTION "content"."reactions_after_update_trigger"() RETURNS "trigger"
    LANGUAGE "plpgsql"
    AS $$begin
  if OLD.target_type <> NEW.target_type
     or OLD.target_id <> NEW.target_id
     or OLD.reaction <> NEW.reaction then
...
ALTER FUNCTION "content"."reactions_after_update_trigger"() OWNER TO "postgres";


CREATE OR REPLACE FUNCTION "content"."refresh_reaction_totals"("_target_type" "text", "_target_id" "uuid") RETURNS "void"
    LANGUAGE "plpgsql"
    AS $$DECLARE
  totals jsonb;
BEGIN
  ...
  IF _target_type = 'prompt' THEN
    UPDATE content.prompt_templates
    SET reaction_totals = totals
    WHERE id = _target_id;
  ELSIF _target_type = 'thread' THEN
    UPDATE content.threads
    SET reaction_totals = totals
    WHERE id = _target_id;
  END IF;
...
ALTER FUNCTION "content"."refresh_reaction_totals"("_target_type" "text", "_target_id" "uuid") OWNER TO "postgres";


CREATE OR REPLACE FUNCTION "content"."set_prompt_author_profile"() RETURNS "trigger"
    LANGUAGE "plpgsql"
    AS $$BEGIN
    NEW.author_profile := lensers.build_author_profile(NEW.lenser_id);
    RETURN NEW;
END;$$;


ALTER FUNCTION "content"."set_prompt_author_profile"() OWNER TO "postgres";


CREATE OR REPLACE FUNCTION "content"."set_thread_author_profile"() RETURNS "trigger"
    LANGUAGE "plpgsql"
    AS $$BEGIN
    NEW.author_profile := lensers.build_author_profile(NEW.lenser_id);
    RETURN NEW;
...
ALTER FUNCTION "content"."set_thread_author_profile"() OWNER TO "postgres";


CREATE OR REPLACE FUNCTION "content"."set_thread_reply_author_profile"() RETURNS "trigger"
    LANGUAGE "plpgsql"
    AS $$BEGIN
    NEW.author_profile := lensers.build_author_profile(NEW.lenser_id);
    RETURN NEW;
...
ALTER FUNCTION "content"."set_thread_reply_author_profile"() OWNER TO "postgres";
...
CREATE TABLE IF NOT EXISTS "content"."thread_replies" (
    ...
    "reaction_totals" "jsonb" DEFAULT '{}'::"jsonb",
    "created_at" timestamp with time zone DEFAULT "now"(),
    "updated_at" timestamp with time zone DEFAULT "now"(),
    "deleted_at" timestamp with time zone,
    "author_profile" "jsonb" DEFAULT '{}'::"jsonb" NOT NULL
);
...
CREATE OR REPLACE FUNCTION "lensers"."prevent_lenser_join_log_delete"() RETURNS "trigger"
    ...
    AS $$
begin
  raise exception 'Deletion from public.lenser_join_log is not allowed';
end;
$$;
...
begin
  if old.join_order <> new.join_order or old.lenser_id <> new.lenser_id then
    raise exception 'join_order and lenser_id are immutable on public.lenser_join_log';
  end if;
...
CREATE OR REPLACE FUNCTION "lensers"."propagate_author_profile_update"() RETURNS "trigger"
    LANGUAGE "plpgsql"
    AS $$DECLARE
    updated_profile jsonb;
BEGIN
    updated_profile := lensers.build_author_profile(NEW.id);

    UPDATE content.threads
    SET author_profile = updated_profile
    WHERE lenser_id = NEW.id;
...
ALTER FUNCTION "lensers"."propagate_author_profile_update"() OWNER TO "postgres";


CREATE OR REPLACE FUNCTION "lensers"."propagate_author_profile_update_to_replies"() RETURNS "trigger"
    LANGUAGE "plpgsql"
    AS $$DECLARE
    updated_profile jsonb;
BEGIN
...
ALTER FUNCTION "lensers"."propagate_author_profile_update_to_replies"() OWNER TO "postgres";
...
DECLARE
    auth_metadata jsonb;
BEGIN
    -- Kullanıcıya ait metadata'yı al
    ...
    IF auth_metadata IS NOT NULL THEN
        ...
        -- Eğer profil alanları NULL ise (yeni oluşturuluyorsa) metadata ile doldur
        IF NEW.preferred_language IS NULL THEN
            NEW.preferred_language := auth_metadata->>'preferred_language';
        END IF;

        IF NEW.ui_language IS NULL THEN
            NEW.ui_language := auth_metadata->>'ui_language';
        END IF;

        IF NEW.detected_language IS NULL THEN
            NEW.detected_language := auth_metadata->>'detected_language';
        END IF;
...
CREATE OR REPLACE FUNCTION "lensers"."trg_award_founder_badges"() RETURNS "trigger"
    ...
    AS $$
BEGIN
  PERFORM public.auto_award_badges_from_join_order(NEW.lenser_id);
  RETURN NEW;
END;
$$;
...
CREATE OR REPLACE FUNCTION "public"."fn_content_create_prompt_template"("p_title" "text", "p_description" "text", "p_content" "text", "p_visibility" "content"."visibility_enum", "p_tag_ids" "uuid"[]) RETURNS "uuid"
    LANGUAGE "plpgsql" SECURITY DEFINER
    SET "search_path" TO 'public', 'auth', 'lensers', 'content'
    AS $$
...
BEGIN
  ...
  SELECT id INTO v_lenser_id
  FROM lensers.profiles      -- fully qualified schema
  WHERE user_id = v_uid
    AND status = 'active'
    AND deletion_requested_at IS NULL;

  IF v_lenser_id IS NULL THEN
    RAISE EXCEPTION 'Active Lenser profile not found';
  END IF;

  INSERT INTO content.prompt_templates (  -- explicit schema reference
    lenser_id, title, description, content, visibility,
    reaction_totals, author_profile
  )
  VALUES (
    v_lenser_id, p_title, p_description, p_content, p_visibility,
    '{}'::jsonb, '{}'::jsonb
  )
  RETURNING id INTO v_prompt_id;

  IF p_tag_ids IS NOT NULL THEN
    INSERT INTO content.tag_map (entity_type, entity_id, tag_id, user_id)
    SELECT 'prompt_template', v_prompt_id, unnest(p_tag_ids), v_uid;
  END IF;
...
ALTER FUNCTION "public"."fn_content_create_prompt_template"("p_title" "text", "p_description" "text", "p_content" "text", "p_visibility" "content"."visibility_enum", "p_tag_ids" "uuid"[]) OWNER TO "postgres";


CREATE OR REPLACE FUNCTION "public"."fn_content_create_reply"("p_thread_id" "uuid", "p_content" "text", "p_parent_reply_id" "uuid" DEFAULT NULL::"uuid") RETURNS "uuid"
    LANGUAGE "plpgsql" SECURITY DEFINER
...
BEGIN
  ...
  PERFORM 1 FROM content.threads WHERE id = p_thread_id;
  IF NOT FOUND THEN
  ...
  INSERT INTO content.thread_replies (
    thread_id, parent_reply_id, lenser_id, content
  )
  VALUES (
    p_thread_id, p_parent_reply_id, v_lenser_id, p_content
  )
...
ALTER FUNCTION "public"."fn_content_create_reply"("p_thread_id" "uuid", "p_content" "text", "p_parent_reply_id" "uuid") OWNER TO "postgres";


CREATE OR REPLACE FUNCTION "public"."fn_content_create_thread"("p_title" "text", "p_content" "text", "p_visibility" "content"."visibility_enum", "p_tag_ids" "uuid"[] DEFAULT NULL::"uuid"[]) RETURNS "uuid"
    LANGUAGE "plpgsql" SECURITY DEFINER
    SET "search_path" TO 'public', 'content', 'lensers', 'auth'
    AS $$
DECLARE
...
BEGIN
  ...
  INSERT INTO content.threads (
    lenser_id, title, content, visibility, reaction_totals
  )
  VALUES (
    v_lenser_id, p_title, p_content, p_visibility, '{}'::jsonb
  )
  ...
  IF p_tag_ids IS NOT NULL AND array_length(p_tag_ids, 1) > 0 THEN
    INSERT INTO content.tag_map (entity_type, entity_id, tag_id, user_id)
    SELECT
      'thread'::content.tag_entity_type,   -- FIXED ENUM
      v_thread_id,
      unnest(p_tag_ids),
      auth.uid();
  END IF;
...
CREATE OR REPLACE FUNCTION "public"."fn_content_delete_prompt"("p_id" "uuid") RETURNS "void"
    LANGUAGE "plpgsql" SECURITY DEFINER
    SET "search_path" TO 'public', 'content', 'lensers', 'auth'
    AS $$
DECLARE
...
BEGIN
  ...
  SELECT id INTO v_lenser_id
  FROM lensers.profiles
  WHERE user_id = v_uid AND status='active' AND deletion_requested_at IS NULL;

  -- Delete prompt
  DELETE FROM content.prompt_templates
  WHERE id = p_id AND lenser_id = v_lenser_id;

  -- Delete tag mappings
  DELETE FROM content.tag_map
  WHERE entity_type='prompt_template'
    AND entity_id=p_id;
END;
$$;


ALTER FUNCTION "public"."fn_content_delete_prompt"("p_id" "uuid") OWNER TO "postgres";


CREATE OR REPLACE FUNCTION "public"."fn_content_delete_reply"("p_reply_id" "uuid") RETURNS "void"
    LANGUAGE "plpgsql" SECURITY DEFINER
    SET "search_path" TO 'public', 'content', 'lensers', 'auth'
    AS $$
DECLARE
...
BEGIN
  ...
  IF v_owner != (
    ...
    RAISE EXCEPTION 'You do not have permission to delete this reply';
  END IF;
...
ALTER FUNCTION "public"."fn_content_delete_reply"("p_reply_id" "uuid") OWNER TO "postgres";


CREATE OR REPLACE FUNCTION "public"."fn_content_delete_thread"("p_thread_id" "uuid") RETURNS "void"
    LANGUAGE "plpgsql" SECURITY DEFINER
    SET "search_path" TO 'public', 'content', 'lensers', 'auth'
    AS $$
DECLARE
  v_owner uuid;
BEGIN
  -- Check thread exists and fetch owner
  SELECT lenser_id INTO v_owner
  ...
  -- Perform delete, replies will cascade
  DELETE FROM content.threads
  WHERE id = p_thread_id;
...
ALTER FUNCTION "public"."fn_content_delete_thread"("p_thread_id" "uuid") OWNER TO "postgres";
...
CREATE TABLE IF NOT EXISTS "content"."reactions" (
    ...
    CONSTRAINT "reactions_target_type_check" CHECK (("target_type" = ANY (ARRAY['thread'::"text", 'thread_reply'::"text", 'prompt_template'::"text"])))
);
...
CREATE OR REPLACE FUNCTION "public"."fn_content_reactions_get_batch_user"("p_target_type" "text", "p_target_ids" "uuid"[]) RETURNS SETOF "content"."reactions"
    LANGUAGE "plpgsql" SECURITY DEFINER
    SET "search_path" TO 'public', 'content', 'lensers', 'auth'
    AS $$
DECLARE
...
BEGIN
  ...
  v_lenser_id := public.fn_lensers_get_current_id();
...
ALTER FUNCTION "public"."fn_content_reactions_get_batch_user"("p_target_type" "text", "p_target_ids" "uuid"[]) OWNER TO "postgres";


CREATE OR REPLACE FUNCTION "public"."fn_content_reactions_get_for_target"("p_target_type" "text", "p_target_id" "uuid") RETURNS SETOF "content"."reactions"
...
ALTER FUNCTION "public"."fn_content_reactions_get_for_target"("p_target_type" "text", "p_target_id" "uuid") OWNER TO "postgres";


CREATE OR REPLACE FUNCTION "public"."fn_content_reactions_get_lenser_history"("p_handle" "text", "p_offset" integer DEFAULT 0, "p_limit" integer DEFAULT 20) RETURNS TABLE("id" "uuid", "lenser_id" "uuid", "target_type" "text", "target_id" "uuid", "reaction" "content"."reaction_enum", "created_at" timestamp with time zone)
    LANGUAGE "plpgsql" SECURITY DEFINER
...
ALTER FUNCTION "public"."fn_content_reactions_get_lenser_history"("p_handle" "text", "p_offset" integer, "p_limit" integer) OWNER TO "postgres";


CREATE OR REPLACE FUNCTION "public"."fn_content_reactions_get_summary"("p_target_type" "text", "p_target_id" "uuid", "p_lenser_id" "uuid") RETURNS "jsonb"
    LANGUAGE "plpgsql" SECURITY DEFINER
    SET "search_path" TO 'public', 'content'
    AS $$
DECLARE
...
BEGIN
  -- Validate target_type early for clearer errors
  IF p_target_type NOT IN ('thread', 'thread_reply', 'prompt_template') THEN
    RAISE EXCEPTION 'Invalid target_type: %', p_target_type
      USING ERRCODE = '22023',   -- invalid_parameter_value
            DETAIL  = 'Allowed: thread, thread_reply, prompt_template';
  END IF;

  -- Aggregate counts per reaction type
  SELECT
    COALESCE(
      jsonb_object_agg(reaction::text, cnt),
      '{}'::jsonb
    ),
    COALESCE(SUM(cnt), 0)
  ...
  -- User reactions for this target
  IF p_lenser_id IS NOT NULL THEN
    SELECT COALESCE(array_agg(r.reaction::text), '{}')
    INTO v_user_reactions
    FROM content.reactions r
    WHERE r.target_type = p_target_type
      AND r.target_id   = p_target_id
      AND r.lenser_id   = p_lenser_id;
  END IF;
...
ALTER FUNCTION "public"."fn_content_reactions_get_summary"("p_target_type" "text", "p_target_id" "uuid", "p_lenser_id" "uuid") OWNER TO "postgres";


CREATE OR REPLACE FUNCTION "public"."fn_content_reactions_get_user_for_target"("p_target_type" "text", "p_target_id" "uuid") RETURNS SETOF "content"."reactions"
    LANGUAGE "plpgsql" SECURITY DEFINER
    SET "search_path" TO 'public', 'content', 'lensers', 'auth'
    AS $$
DECLARE
...
BEGIN
  ...
  v_lenser_id := public.fn_lensers_get_current_id();
  IF v_lenser_id IS NULL THEN
    RAISE EXCEPTION 'No active Lenser profile associated with this user';
  END IF;

  -- validate type using enum type content.entity_type_enum
  BEGIN
    PERFORM p_target_type::content.entity_type_enum;
  EXCEPTION WHEN invalid_text_representation THEN
    RAISE EXCEPTION 'Invalid target_type %', p_target_type;
  END;

  -- special unlimited: copy
  IF p_reaction = 'copy'::content.reaction_enum THEN
    INSERT INTO content.reactions (lenser_id, target_type, target_id, reaction)
    VALUES (v_lenser_id, p_target_type, p_target_id, p_reaction);
    RETURN jsonb_build_object('status','copied');
  END IF;

  -- toggle for a defined set of reaction enums
  SELECT id
  INTO v_existing_id
  FROM content.reactions
  WHERE lenser_id   = v_lenser_id
    ...
    AND target_id   = p_target_id
    AND reaction IN (
      'like'::content.reaction_enum,
      'dislike'::content.reaction_enum,
      'saved'::content.reaction_enum
    );
...
declare
  v_id uuid;
begin
  -- Basic validation (DB constraints still apply)
  ...
  insert into content.tags (name, slug, visibility)
  values (btrim(p_name), btrim(p_slug), 'public')
...
CREATE OR REPLACE FUNCTION "public"."fn_content_update_prompt"("p_id" "uuid", "p_title" "text" DEFAULT NULL::"text", "p_description" "text" DEFAULT NULL::"text", "p_content" "text" DEFAULT NULL::"text", "p_visibility" "content"."visibility_enum" DEFAULT NULL::"content"."visibility_enum", "p_tag_ids" "uuid"[] DEFAULT NULL::"uuid"[]) RETURNS "void"
    LANGUAGE "plpgsql" SECURITY DEFINER
    SET "search_path" TO 'public', 'content', 'lensers', 'auth'
    AS $$
DECLARE
  v_uid       uuid;
  v_lenser_id uuid;
  v_prompt_visibility content.visibility_enum;
  v_tag_ids   uuid[];
  v_system_tag integer;
  v_invalid_private_tag integer;
  v_invalid_public_tag integer;
BEGIN
  v_uid := auth.uid();
  IF v_uid IS NULL THEN
    RAISE EXCEPTION 'Authentication required';
  ...
  -- Final visibility after update
  v_prompt_visibility := COALESCE(p_visibility, v_prompt_visibility);

  -- ============ Tag Validation ============
  IF p_tag_ids IS NOT NULL THEN
    v_tag_ids := p_tag_ids;

    -- System tags cannot be used manually
    SELECT COUNT(*)
    INTO v_system_tag
    FROM content.tags
    ...
    IF v_system_tag > 0 THEN
      RAISE EXCEPTION 'System tags cannot be assigned manually';
    END IF;

    IF v_prompt_visibility = 'public' THEN
      -- Public prompts cannot use private tags
      SELECT COUNT(*)
      INTO v_invalid_public_tag
      FROM content.tags
      WHERE id = ANY (v_tag_ids)
        AND visibility <> 'public';

      IF v_invalid_public_tag > 0 THEN
        RAISE EXCEPTION 'This prompt is public, but one or more tags are private/restricted';
      END IF;
    END IF;
  END IF;
  ...
  SET title       = COALESCE(p_title, title),
      ...
      visibility  = v_prompt_visibility,
      updated_at  = now()
  WHERE id = p_id AND lenser_id = v_lenser_id;

  -- ============ UPDATE TAGS (replace set) ============
  IF p_tag_ids IS NOT NULL THEN
    DELETE FROM content.tag_map
    WHERE entity_type = 'prompt_template'
      AND entity_id = p_id;
    ...
    IF array_length(v_tag_ids,1) > 0 THEN
      INSERT INTO content.tag_map (entity_type, entity_id, tag_id, user_id)
      SELECT 'prompt_template', p_id, unnest(v_tag_ids), v_uid;
    END IF;
  END IF;
...
ALTER FUNCTION "public"."fn_content_update_prompt"("p_id" "uuid", "p_title" "text", "p_description" "text", "p_content" "text", "p_visibility" "content"."visibility_enum", "p_tag_ids" "uuid"[]) OWNER TO "postgres";


CREATE OR REPLACE FUNCTION "public"."fn_content_update_thread"("p_thread_id" "uuid", "p_title" "text" DEFAULT NULL::"text", "p_content" "text" DEFAULT NULL::"text", "p_visibility" "content"."visibility_enum" DEFAULT NULL::"content"."visibility_enum", "p_tag_ids" "uuid"[] DEFAULT NULL::"uuid"[]) RETURNS "void"
    LANGUAGE "plpgsql" SECURITY DEFINER
    SET "search_path" TO 'public', 'content', 'lensers', 'auth'
    AS $$
DECLARE
...
BEGIN
  ...
  IF v_owner != (
    ...
    RAISE EXCEPTION 'You do not have permission to update this thread';
  END IF;
  ...
  -- Replace tags with correct enum type
  IF p_tag_ids IS NOT NULL THEN

    DELETE FROM content.tag_map
    WHERE entity_type = 'thread'::content.tag_entity_type  -- FIX
...
ALTER FUNCTION "public"."fn_content_update_thread"("p_thread_id" "uuid", "p_title" "text", "p_content" "text", "p_visibility" "content"."visibility_enum", "p_tag_ids" "uuid"[]) OWNER TO "postgres";


CREATE OR REPLACE FUNCTION "public"."fn_lensers_create_profile"("p_handle" "text", "p_display_name" "text", "p_bio" "text" DEFAULT NULL::"text", "p_headline" "text" DEFAULT NULL::"text") RETURNS TABLE("out_lenser_id" "uuid", "out_user_id" "uuid", "out_handle" "text", "out_display_name" "text", "out_status" "lensers"."lenser_status", "out_join_order" bigint)
    LANGUAGE "plpgsql" SECURITY DEFINER
    AS $$DECLARE
  v_uid uuid;
  v_lenser RECORD;
...
BEGIN
  ...
  -- 2) Bu kullanıcı zaten Lenser profiline sahip mi?
  IF EXISTS (SELECT 1 FROM lensers.profiles WHERE user_id = v_uid) THEN
    RAISE EXCEPTION 'A Lenser profile already exists for this user.';
  END IF;

  -- 3) Lenser oluştur
  INSERT INTO lensers.profiles (user_id, handle, display_name, bio, headline)
  VALUES (v_uid, p_handle, p_display_name, p_bio, p_headline)
  RETURNING * INTO v_lenser;

  -- 4) Join order tetikleyiciyle oluşturuldu – şimdi çekiyoruz
  SELECT join_order
  INTO v_log
  FROM analytics.lenser_join_log
  WHERE lenser_id = v_lenser.id;

  -- 5) Dönüş değerleri
  out_lenser_id    := v_lenser.id;
  out_user_id      := v_lenser.user_id;
  out_handle       := v_lenser.handle;
  out_display_name := v_lenser.display_name;
  out_status       := v_lenser.status;
  out_join_order   := v_log.join_order;

  RETURN NEXT;
END;$$;


ALTER FUNCTION "public"."fn_lensers_create_profile"("p_handle" "text", "p_display_name" "text", "p_bio" "text", "p_headline" "text") OWNER TO "postgres";


CREATE OR REPLACE FUNCTION "public"."fn_lensers_delete_social_link"("p_id" "uuid") RETURNS "void"
    LANGUAGE "plpgsql" SECURITY DEFINER
...
ALTER FUNCTION "public"."fn_lensers_delete_social_link"("p_id" "uuid") OWNER TO "postgres";
...
CREATE OR REPLACE FUNCTION "public"."fn_lensers_insert_social_link"("p_platform" "lensers"."lenser_social_platform", "p_url" "text", "p_label" "text") RETURNS "void"
    LANGUAGE "plpgsql" SECURITY DEFINER
    SET "search_path" TO 'public', 'lensers', 'auth'
    AS $$
DECLARE 
...
BEGIN
  ...
  INSERT INTO lensers.social_links (lenser_id, platform, url, label)
  VALUES (v_lenser_id, p_platform, p_url, p_label);
END;
$$;


ALTER FUNCTION "public"."fn_lensers_insert_social_link"("p_platform" "lensers"."lenser_social_platform", "p_url" "text", "p_label" "text") OWNER TO "postgres";
...
CREATE OR REPLACE FUNCTION "public"."fn_lensers_update_profile"("p_display_name" "text" DEFAULT NULL::"text", "p_avatar_url" "text" DEFAULT NULL::"text", "p_banner_url" "text" DEFAULT NULL::"text", "p_bio" "text" DEFAULT NULL::"text", "p_headline" "text" DEFAULT NULL::"text", "p_preferences" "jsonb" DEFAULT NULL::"jsonb") RETURNS "jsonb"
    LANGUAGE "plpgsql" SECURITY DEFINER
    SET "search_path" TO 'public', 'lensers', 'auth'
    AS $$
DECLARE
...
ALTER FUNCTION "public"."fn_lensers_update_profile"("p_display_name" "text", "p_avatar_url" "text", "p_banner_url" "text", "p_bio" "text", "p_headline" "text", "p_preferences" "jsonb") OWNER TO "postgres";


CREATE OR REPLACE FUNCTION "public"."fn_lensers_update_social_link"("p_id" "uuid", "p_platform" "lensers"."lenser_social_platform", "p_url" "text", "p_label" "text") RETURNS "void"
    LANGUAGE "plpgsql" SECURITY DEFINER
    SET "search_path" TO 'public', 'lensers', 'auth'
    AS $$
BEGIN
  UPDATE lensers.social_links
  SET 
    platform = p_platform,
    url      = p_url,
    label    = p_label
...
ALTER FUNCTION "public"."fn_lensers_update_social_link"("p_id" "uuid", "p_platform" "lensers"."lenser_social_platform", "p_url" "text", "p_label" "text") OWNER TO "postgres";


CREATE OR REPLACE FUNCTION "public"."fn_lensers_update_theme"("p_theme" "text") RETURNS "void"
    LANGUAGE "plpgsql"
    AS $$
DECLARE
  v_uid uuid := auth.uid();
BEGIN
  ...
  -- must be an active lenser, otherwise reject
  IF NOT lensers.is_active_lenser(v_uid) THEN
    RAISE EXCEPTION 'Lenser profile required';
  END IF;
...
ALTER FUNCTION "public"."fn_lensers_update_theme"("p_theme" "text") OWNER TO "postgres";
...
CREATE OR REPLACE FUNCTION "public"."fn_ops_create_contact"("p_name" "text", "p_email" "text", "p_subject" "text", "p_message" "text", "p_kvkk_approved" boolean, "p_ip_address" "inet", "p_user_agent" "text") RETURNS "void"
    LANGUAGE "plpgsql" SECURITY DEFINER
    SET "search_path" TO 'public', 'auth', 'ops'
    AS $$
DECLARE
  v_user_id uuid;
  v_lenser_id uuid;
BEGIN
  IF p_kvkk_approved IS NOT TRUE THEN
    RAISE EXCEPTION 'KVKK approval is required.';
  END IF;

  -- Extract authenticated user_id if exists
  v_user_id := auth.uid();

  -- If authenticated but no profile exists → create minimal profile
  IF v_user_id IS NOT NULL THEN
    SELECT id
    INTO v_lenser_id
    FROM lensers.profiles
    ...
    IF v_lenser_id IS NULL THEN
      INSERT INTO lensers.profiles (user_id, handle, display_name)
      VALUES (
        v_user_id,
        'user_' || substr(v_user_id::text, 1, 8),
        'User ' || substr(v_user_id::text, 1, 6)
      )
      RETURNING id INTO v_lenser_id;
    END IF;
  END IF;

  -- Insert into ops.contact
  INSERT INTO ops.contact (
    lenser_id,
    name,
    email,
...
ALTER FUNCTION "public"."fn_ops_create_contact"("p_name" "text", "p_email" "text", "p_subject" "text", "p_message" "text", "p_kvkk_approved" boolean, "p_ip_address" "inet", "p_user_agent" "text") OWNER TO "postgres";


CREATE OR REPLACE FUNCTION "public"."fn_public_submit_feedback"("p_product_tag" "text", "p_page" "text", "p_message" "text", "p_start_date" timestamp with time zone, "p_end_date" timestamp with time zone) RETURNS "void"
    LANGUAGE "plpgsql" SECURITY DEFINER
    SET "search_path" TO 'public'
    AS $$
DECLARE
  v_user_id uuid := auth.uid();
  v_tag analytics.product_tag_enum;
BEGIN
  --------------------------------------------------------------------
  -- VALIDATION: product_tag → internal enum
  --------------------------------------------------------------------
  IF p_product_tag IS NULL OR btrim(p_product_tag) = '' THEN
    v_tag := 'general'::analytics.product_tag_enum;
  ELSE
    BEGIN
      v_tag := p_product_tag::analytics.product_tag_enum;
    EXCEPTION
      WHEN invalid_text_representation THEN
        RAISE EXCEPTION 'Invalid product_tag: %', p_product_tag
          USING ERRCODE = '22023'; -- invalid parameter value
    END;
  END IF;

  --------------------------------------------------------------------
  -- VALIDATION: page must NOT be empty
  --------------------------------------------------------------------
  ...
  --------------------------------------------------------------------
  -- INSERT (secure; only function owner needs access to analytics)
  --------------------------------------------------------------------
  INSERT INTO analytics.product_feedback (
    product_tag,
    page,
    user_id,
    message,
  ...
  -- no RETURN needed; RETURNS void

END;
$$;


ALTER FUNCTION "public"."fn_public_submit_feedback"("p_product_tag" "text", "p_page" "text", "p_message" "text", "p_start_date" timestamp with time zone, "p_end_date" timestamp with time zone) OWNER TO "postgres";
...
CREATE TABLE IF NOT EXISTS "content"."prompt_templates" (
    ...
    "author_profile" "jsonb" DEFAULT '{}'::"jsonb" NOT NULL,
    CONSTRAINT "prompt_content_check" CHECK ((("btrim"("content") = "content") AND (("char_length"("content") >= 10) AND ("char_length"("content") <= 20000)))),
    CONSTRAINT "prompt_description_check" CHECK ((("description" IS NULL) OR (("btrim"("description") = "description") AND (("char_length"("description") >= 0) AND ("char_length"("description") <= 300))))),
    CONSTRAINT "prompt_title_check" CHECK ((("btrim"("title") = "title") AND (("char_length"("title") >= 3) AND ("char_length"("title") <= 120))))
);
...
CREATE TABLE IF NOT EXISTS "content"."tags" (
    ...
    CONSTRAINT "tags_name_format_check" CHECK ((("btrim"("name") = "name") AND (("char_length"("name") >= 2) AND ("char_length"("name") <= 60)))),
...
CREATE TABLE IF NOT EXISTS "content"."threads" (
    ...
    "reaction_totals" "jsonb" DEFAULT '{}'::"jsonb" NOT NULL,
    "author_profile" "jsonb" DEFAULT '{}'::"jsonb" NOT NULL,
    CONSTRAINT "threads_content_length_check" CHECK ((("char_length"("content") >= 1) AND ("char_length"("content") <= 20000))),
    CONSTRAINT "threads_title_length_check" CHECK ((("char_length"("title") >= 1) AND ("char_length"("title") <= 200)))
);
...
CREATE OR REPLACE VIEW "public"."vw_content_threads_public" AS
 SELECT "t"."id",
    ...
    "t"."created_at",
    "t"."thumbnail_url",
    COALESCE("jsonb_agg"("jsonb_build_object"('slug', "tg"."slug", 'name', "tg"."name")) FILTER (WHERE ("tg"."id" IS NOT NULL)), '[]'::"jsonb") AS "tags"
   FROM (("content"."threads" "t"
     LEFT JOIN "content"."tag_map" "tm" ON ((("tm"."entity_type" = 'thread'::"content"."entity_type_enum") AND ("tm"."entity_id" = "t"."id"))))
     LEFT JOIN "content"."tags" "tg" ON (("tg"."id" = "tm"."tag_id")))
  WHERE ("t"."visibility" = 'public'::"content"."visibility_enum")
  GROUP BY "t"."id", "t"."title", "t"."content", "t"."author_profile", "t"."reaction_totals", "t"."reply_count", "t"."created_at", "t"."thumbnail_url";
...
CREATE OR REPLACE VIEW "public"."vw_prompt_templates_public" AS
 SELECT "pt"."id",
    ...
    COALESCE("jsonb_agg"("jsonb_build_object"('slug', "tg"."slug", 'name', "tg"."name")) FILTER (WHERE ("tg"."id" IS NOT NULL)), '[]'::"jsonb") AS "tags"
   FROM (("content"."prompt_templates" "pt"
     LEFT JOIN "content"."tag_map" "tm" ON ((("tm"."entity_type" = 'prompt_template'::"content"."entity_type_enum") AND ("tm"."entity_id" = "pt"."id"))))
     LEFT JOIN "content"."tags" "tg" ON (("tg"."id" = "tm"."tag_id")))
  WHERE ("pt"."visibility" = 'public'::"content"."visibility_enum")
  GROUP BY "pt"."id", "pt"."title", "pt"."description", "pt"."content", "pt"."author_profile", "pt"."reaction_totals", "pt"."created_at";
...
CREATE INDEX "reactions_lenser_idx" ON "content"."reactions" USING "btree" ("lenser_id");



CREATE INDEX "reactions_reaction_idx" ON "content"."reactions" USING "btree" ("reaction");



CREATE INDEX "reactions_target_idx" ON "content"."reactions" USING "btree" ("target_type", "target_id");



CREATE UNIQUE INDEX "reactions_unique_limit_index" ON "content"."reactions" USING "btree" ("lenser_id", "target_type", "target_id") WHERE ("reaction" = ANY (ARRAY['like'::"content"."reaction_enum", 'dislike'::"content"."reaction_enum", 'saved'::"content"."reaction_enum"]));
...
CREATE OR REPLACE TRIGGER "prompt_template_reaction_after_delete" AFTER DELETE ON "content"."reactions" FOR EACH ROW EXECUTE FUNCTION "content"."update_prompt_template_reaction_counters"();



CREATE OR REPLACE TRIGGER "prompt_template_reaction_after_insert" AFTER INSERT ON "content"."reactions" FOR EACH ROW EXECUTE FUNCTION "content"."update_prompt_template_reaction_counters"();



CREATE OR REPLACE TRIGGER "prompt_template_reaction_after_update" AFTER UPDATE ON "content"."reactions" FOR EACH ROW EXECUTE FUNCTION "content"."update_prompt_template_reaction_counters"();



CREATE OR REPLACE TRIGGER "reactions_after_delete" AFTER DELETE ON "content"."reactions" FOR EACH ROW EXECUTE FUNCTION "content"."reactions_after_delete_trigger"();



CREATE OR REPLACE TRIGGER "reactions_after_insert" AFTER INSERT ON "content"."reactions" FOR EACH ROW EXECUTE FUNCTION "content"."reactions_after_insert_trigger"();



CREATE OR REPLACE TRIGGER "reactions_after_update" AFTER UPDATE ON "content"."reactions" FOR EACH ROW EXECUTE FUNCTION "content"."reactions_after_update_trigger"();
...
CREATE OR REPLACE TRIGGER "trg_block_protected_fields" BEFORE UPDATE ON "content"."prompt_templates" FOR EACH ROW EXECUTE FUNCTION "content"."prevent_protected_prompt_fields"();



CREATE OR REPLACE TRIGGER "trg_prompt_set_author_profile" BEFORE INSERT ON "content"."prompt_templates" FOR EACH ROW EXECUTE FUNCTION "content"."set_prompt_author_profile"();



CREATE OR REPLACE TRIGGER "trg_reactions_refresh_totals" AFTER INSERT OR DELETE OR UPDATE ON "content"."reactions" FOR EACH ROW EXECUTE FUNCTION "content"."trg_reactions_refresh_totals"();
...
CREATE OR REPLACE TRIGGER "trg_tag_reaction" AFTER INSERT ON "content"."reactions" FOR EACH ROW EXECUTE FUNCTION "xp"."on_reaction_added"();



CREATE OR REPLACE TRIGGER "trg_thread_replies_set_author_profile" BEFORE INSERT ON "content"."thread_replies" FOR EACH ROW EXECUTE FUNCTION "content"."set_thread_reply_author_profile"();



CREATE OR REPLACE TRIGGER "trg_threads_set_author_profile" BEFORE INSERT ON "content"."threads" FOR EACH ROW EXECUTE FUNCTION "content"."set_thread_author_profile"();



CREATE OR REPLACE TRIGGER "trg_xp_on_reaction_given" AFTER INSERT ON "content"."reactions" FOR EACH ROW EXECUTE FUNCTION "xp"."on_reaction_given"();



CREATE OR REPLACE TRIGGER "trg_xp_on_reaction_received" AFTER INSERT ON "content"."reactions" FOR EACH ROW EXECUTE FUNCTION "xp"."on_reaction_received"();
...
CREATE OR REPLACE TRIGGER "trg_lensers_propagate_profile" AFTER UPDATE ON "lensers"."profiles" FOR EACH ROW EXECUTE FUNCTION "lensers"."propagate_author_profile_update"();



CREATE OR REPLACE TRIGGER "trg_lensers_propagate_profile_to_replies" AFTER UPDATE ON "lensers"."profiles" FOR EACH ROW EXECUTE FUNCTION "lensers"."propagate_author_profile_update_to_replies"();
...
ALTER TABLE ONLY "content"."reactions"
    ADD CONSTRAINT "reactions_lenser_id_fkey" FOREIGN KEY ("lenser_id") REFERENCES "lensers"."profiles"("id") ON DELETE SET NULL;



ALTER TABLE ONLY "content"."reactions"
    ADD CONSTRAINT "reactions_user_fk" FOREIGN KEY ("user_id") REFERENCES "auth"."users"("id") ON DELETE CASCADE;
...
CREATE POLICY "ai_generations_admin_select_all" ON "ai"."generations" FOR SELECT TO "authenticated" USING ("public"."is_admin"());
...
CREATE POLICY "ai_generations_select_own" ON "ai"."generations" FOR SELECT TO "authenticated" USING ("lensers"."user_owns_lenser"("lenser_id"));
...
CREATE POLICY "feedback_read_own" ON "analytics"."product_feedback" FOR SELECT USING (("user_id" = "auth"."uid"()));
...
CREATE POLICY "no_direct_access" ON "analytics"."product_feedback" TO "authenticated", "anon" USING (false) WITH CHECK (false);
...
CREATE POLICY "no_direct_modify" ON "content"."tag_map" USING (false) WITH CHECK (false);
...
CREATE POLICY "prompt_templates_select" ON "content"."prompt_templates" FOR SELECT USING ((("auth"."uid"() IS NOT NULL) AND (("lensers"."is_active_lenser"("auth"."uid"()) AND ("lenser_id" = "lensers"."current_active_lenser_id"())) OR ("visibility" = 'public'::"content"."visibility_enum"))));
...
CREATE POLICY "reactions_block_all" ON "content"."reactions" TO "authenticated", "anon" USING (false) WITH CHECK (false);



CREATE POLICY "reactions_owner_delete" ON "content"."reactions" FOR DELETE USING (("lensers"."is_active_lenser"("auth"."uid"()) AND ("lenser_id" = "lensers"."current_active_lenser_id"())));



CREATE POLICY "reactions_owner_insert" ON "content"."reactions" FOR INSERT WITH CHECK (("lensers"."is_active_lenser"("auth"."uid"()) AND ("lenser_id" = "lensers"."current_active_lenser_id"())));



CREATE POLICY "reactions_public_select" ON "content"."reactions" FOR SELECT USING (("target_id" IS NOT NULL));



CREATE POLICY "reactions_service_role_all" ON "content"."reactions" TO "service_role" USING (true) WITH CHECK (true);
...
CREATE POLICY "thread_replies_public_select" ON "content"."thread_replies" FOR SELECT USING (("status" = 'public'::"content"."thread_visibility"));
...
CREATE POLICY "threads_public_select" ON "content"."threads" FOR SELECT USING (("visibility" = 'public'::"content"."visibility_enum"));
...
GRANT ALL ON FUNCTION "content"."adjust_reaction_totals"("_target_type" "text", "_target_id" "uuid", "_reaction" "text", "_delta" integer) TO "anon";
GRANT ALL ON FUNCTION "content"."adjust_reaction_totals"("_target_type" "text", "_target_id" "uuid", "_reaction" "text", "_delta" integer) TO "authenticated";
GRANT ALL ON FUNCTION "content"."adjust_reaction_totals"("_target_type" "text", "_target_id" "uuid", "_reaction" "text", "_delta" integer) TO "service_role";
...
GRANT ALL ON FUNCTION "content"."prevent_protected_prompt_fields"() TO "anon";
GRANT ALL ON FUNCTION "content"."prevent_protected_prompt_fields"() TO "authenticated";
GRANT ALL ON FUNCTION "content"."prevent_protected_prompt_fields"() TO "service_role";



GRANT ALL ON FUNCTION "content"."reactions_after_delete_trigger"() TO "anon";
GRANT ALL ON FUNCTION "content"."reactions_after_delete_trigger"() TO "authenticated";
GRANT ALL ON FUNCTION "content"."reactions_after_delete_trigger"() TO "service_role";



GRANT ALL ON FUNCTION "content"."reactions_after_insert_trigger"() TO "anon";
GRANT ALL ON FUNCTION "content"."reactions_after_insert_trigger"() TO "authenticated";
GRANT ALL ON FUNCTION "content"."reactions_after_insert_trigger"() TO "service_role";



GRANT ALL ON FUNCTION "content"."reactions_after_update_trigger"() TO "anon";
GRANT ALL ON FUNCTION "content"."reactions_after_update_trigger"() TO "authenticated";
GRANT ALL ON FUNCTION "content"."reactions_after_update_trigger"() TO "service_role";



GRANT ALL ON FUNCTION "content"."refresh_reaction_totals"("_target_type" "text", "_target_id" "uuid") TO "anon";
GRANT ALL ON FUNCTION "content"."refresh_reaction_totals"("_target_type" "text", "_target_id" "uuid") TO "authenticated";
GRANT ALL ON FUNCTION "content"."refresh_reaction_totals"("_target_type" "text", "_target_id" "uuid") TO "service_role";



GRANT ALL ON FUNCTION "content"."set_prompt_author_profile"() TO "anon";
GRANT ALL ON FUNCTION "content"."set_prompt_author_profile"() TO "authenticated";
GRANT ALL ON FUNCTION "content"."set_prompt_author_profile"() TO "service_role";



GRANT ALL ON FUNCTION "content"."set_thread_author_profile"() TO "anon";
GRANT ALL ON FUNCTION "content"."set_thread_author_profile"() TO "authenticated";
GRANT ALL ON FUNCTION "content"."set_thread_author_profile"() TO "service_role";



GRANT ALL ON FUNCTION "content"."set_thread_reply_author_profile"() TO "anon";
GRANT ALL ON FUNCTION "content"."set_thread_reply_author_profile"() TO "authenticated";
GRANT ALL ON FUNCTION "content"."set_thread_reply_author_profile"() TO "service_role";
...
GRANT ALL ON FUNCTION "lensers"."propagate_author_profile_update"() TO "anon";
GRANT ALL ON FUNCTION "lensers"."propagate_author_profile_update"() TO "authenticated";
GRANT ALL ON FUNCTION "lensers"."propagate_author_profile_update"() TO "service_role";



GRANT ALL ON FUNCTION "lensers"."propagate_author_profile_update_to_replies"() TO "anon";
GRANT ALL ON FUNCTION "lensers"."propagate_author_profile_update_to_replies"() TO "authenticated";
GRANT ALL ON FUNCTION "lensers"."propagate_author_profile_update_to_replies"() TO "service_role";
...
GRANT ALL ON FUNCTION "public"."fn_content_create_prompt_template"("p_title" "text", "p_description" "text", "p_content" "text", "p_visibility" "content"."visibility_enum", "p_tag_ids" "uuid"[]) TO "anon";
GRANT ALL ON FUNCTION "public"."fn_content_create_prompt_template"("p_title" "text", "p_description" "text", "p_content" "text", "p_visibility" "content"."visibility_enum", "p_tag_ids" "uuid"[]) TO "authenticated";
GRANT ALL ON FUNCTION "public"."fn_content_create_prompt_template"("p_title" "text", "p_description" "text", "p_content" "text", "p_visibility" "content"."visibility_enum", "p_tag_ids" "uuid"[]) TO "service_role";



GRANT ALL ON FUNCTION "public"."fn_content_create_reply"("p_thread_id" "uuid", "p_content" "text", "p_parent_reply_id" "uuid") TO "anon";
GRANT ALL ON FUNCTION "public"."fn_content_create_reply"("p_thread_id" "uuid", "p_content" "text", "p_parent_reply_id" "uuid") TO "authenticated";
GRANT ALL ON FUNCTION "public"."fn_content_create_reply"("p_thread_id" "uuid", "p_content" "text", "p_parent_reply_id" "uuid") TO "service_role";
...
GRANT ALL ON FUNCTION "public"."fn_content_delete_prompt"("p_id" "uuid") TO "anon";
GRANT ALL ON FUNCTION "public"."fn_content_delete_prompt"("p_id" "uuid") TO "authenticated";
GRANT ALL ON FUNCTION "public"."fn_content_delete_prompt"("p_id" "uuid") TO "service_role";



GRANT ALL ON FUNCTION "public"."fn_content_delete_reply"("p_reply_id" "uuid") TO "anon";
GRANT ALL ON FUNCTION "public"."fn_content_delete_reply"("p_reply_id" "uuid") TO "authenticated";
GRANT ALL ON FUNCTION "public"."fn_content_delete_reply"("p_reply_id" "uuid") TO "service_role";



GRANT ALL ON FUNCTION "public"."fn_content_delete_thread"("p_thread_id" "uuid") TO "anon";
GRANT ALL ON FUNCTION "public"."fn_content_delete_thread"("p_thread_id" "uuid") TO "authenticated";
GRANT ALL ON FUNCTION "public"."fn_content_delete_thread"("p_thread_id" "uuid") TO "service_role";
...
GRANT ALL ON FUNCTION "public"."fn_content_reactions_get_batch_user"("p_target_type" "text", "p_target_ids" "uuid"[]) TO "anon";
GRANT ALL ON FUNCTION "public"."fn_content_reactions_get_batch_user"("p_target_type" "text", "p_target_ids" "uuid"[]) TO "authenticated";
GRANT ALL ON FUNCTION "public"."fn_content_reactions_get_batch_user"("p_target_type" "text", "p_target_ids" "uuid"[]) TO "service_role";



GRANT ALL ON FUNCTION "public"."fn_content_reactions_get_for_target"("p_target_type" "text", "p_target_id" "uuid") TO "anon";
GRANT ALL ON FUNCTION "public"."fn_content_reactions_get_for_target"("p_target_type" "text", "p_target_id" "uuid") TO "authenticated";
GRANT ALL ON FUNCTION "public"."fn_content_reactions_get_for_target"("p_target_type" "text", "p_target_id" "uuid") TO "service_role";



GRANT ALL ON FUNCTION "public"."fn_content_reactions_get_lenser_history"("p_handle" "text", "p_offset" integer, "p_limit" integer) TO "anon";
GRANT ALL ON FUNCTION "public"."fn_content_reactions_get_lenser_history"("p_handle" "text", "p_offset" integer, "p_limit" integer) TO "authenticated";
GRANT ALL ON FUNCTION "public"."fn_content_reactions_get_lenser_history"("p_handle" "text", "p_offset" integer, "p_limit" integer) TO "service_role";



GRANT ALL ON FUNCTION "public"."fn_content_reactions_get_summary"("p_target_type" "text", "p_target_id" "uuid", "p_lenser_id" "uuid") TO "anon";
GRANT ALL ON FUNCTION "public"."fn_content_reactions_get_summary"("p_target_type" "text", "p_target_id" "uuid", "p_lenser_id" "uuid") TO "authenticated";
GRANT ALL ON FUNCTION "public"."fn_content_reactions_get_summary"("p_target_type" "text", "p_target_id" "uuid", "p_lenser_id" "uuid") TO "service_role";
...
REVOKE ALL ON FUNCTION "public"."fn_content_tags_create"("p_name" "text", "p_slug" "text") FROM PUBLIC;
...
GRANT ALL ON FUNCTION "public"."fn_content_update_prompt"("p_id" "uuid", "p_title" "text", "p_description" "text", "p_content" "text", "p_visibility" "content"."visibility_enum", "p_tag_ids" "uuid"[]) TO "anon";
GRANT ALL ON FUNCTION "public"."fn_content_update_prompt"("p_id" "uuid", "p_title" "text", "p_description" "text", "p_content" "text", "p_visibility" "content"."visibility_enum", "p_tag_ids" "uuid"[]) TO "authenticated";
GRANT ALL ON FUNCTION "public"."fn_content_update_prompt"("p_id" "uuid", "p_title" "text", "p_description" "text", "p_content" "text", "p_visibility" "content"."visibility_enum", "p_tag_ids" "uuid"[]) TO "service_role";



GRANT ALL ON FUNCTION "public"."fn_content_update_thread"("p_thread_id" "uuid", "p_title" "text", "p_content" "text", "p_visibility" "content"."visibility_enum", "p_tag_ids" "uuid"[]) TO "anon";
GRANT ALL ON FUNCTION "public"."fn_content_update_thread"("p_thread_id" "uuid", "p_title" "text", "p_content" "text", "p_visibility" "content"."visibility_enum", "p_tag_ids" "uuid"[]) TO "authenticated";
GRANT ALL ON FUNCTION "public"."fn_content_update_thread"("p_thread_id" "uuid", "p_title" "text", "p_content" "text", "p_visibility" "content"."visibility_enum", "p_tag_ids" "uuid"[]) TO "service_role";



REVOKE ALL ON FUNCTION "public"."fn_lensers_create_profile"("p_handle" "text", "p_display_name" "text", "p_bio" "text", "p_headline" "text") FROM PUBLIC;
GRANT ALL ON FUNCTION "public"."fn_lensers_create_profile"("p_handle" "text", "p_display_name" "text", "p_bio" "text", "p_headline" "text") TO "anon";
GRANT ALL ON FUNCTION "public"."fn_lensers_create_profile"("p_handle" "text", "p_display_name" "text", "p_bio" "text", "p_headline" "text") TO "authenticated";
GRANT ALL ON FUNCTION "public"."fn_lensers_create_profile"("p_handle" "text", "p_display_name" "text", "p_bio" "text", "p_headline" "text") TO "service_role";



REVOKE ALL ON FUNCTION "public"."fn_lensers_delete_social_link"("p_id" "uuid") FROM PUBLIC;
GRANT ALL ON FUNCTION "public"."fn_lensers_delete_social_link"("p_id" "uuid") TO "anon";
GRANT ALL ON FUNCTION "public"."fn_lensers_delete_social_link"("p_id" "uuid") TO "authenticated";
GRANT ALL ON FUNCTION "public"."fn_lensers_delete_social_link"("p_id" "uuid") TO "service_role";
...
REVOKE ALL ON FUNCTION "public"."fn_lensers_insert_social_link"("p_platform" "lensers"."lenser_social_platform", "p_url" "text", "p_label" "text") FROM PUBLIC;
GRANT ALL ON FUNCTION "public"."fn_lensers_insert_social_link"("p_platform" "lensers"."lenser_social_platform", "p_url" "text", "p_label" "text") TO "anon";
GRANT ALL ON FUNCTION "public"."fn_lensers_insert_social_link"("p_platform" "lensers"."lenser_social_platform", "p_url" "text", "p_label" "text") TO "authenticated";
GRANT ALL ON FUNCTION "public"."fn_lensers_insert_social_link"("p_platform" "lensers"."lenser_social_platform", "p_url" "text", "p_label" "text") TO "service_role";
...
GRANT ALL ON FUNCTION "public"."fn_lensers_update_profile"("p_display_name" "text", "p_avatar_url" "text", "p_banner_url" "text", "p_bio" "text", "p_headline" "text", "p_preferences" "jsonb") TO "anon";
GRANT ALL ON FUNCTION "public"."fn_lensers_update_profile"("p_display_name" "text", "p_avatar_url" "text", "p_banner_url" "text", "p_bio" "text", "p_headline" "text", "p_preferences" "jsonb") TO "authenticated";
GRANT ALL ON FUNCTION "public"."fn_lensers_update_profile"("p_display_name" "text", "p_avatar_url" "text", "p_banner_url" "text", "p_bio" "text", "p_headline" "text", "p_preferences" "jsonb") TO "service_role";



REVOKE ALL ON FUNCTION "public"."fn_lensers_update_social_link"("p_id" "uuid", "p_platform" "lensers"."lenser_social_platform", "p_url" "text", "p_label" "text") FROM PUBLIC;
GRANT ALL ON FUNCTION "public"."fn_lensers_update_social_link"("p_id" "uuid", "p_platform" "lensers"."lenser_social_platform", "p_url" "text", "p_label" "text") TO "anon";
GRANT ALL ON FUNCTION "public"."fn_lensers_update_social_link"("p_id" "uuid", "p_platform" "lensers"."lenser_social_platform", "p_url" "text", "p_label" "text") TO "authenticated";
GRANT ALL ON FUNCTION "public"."fn_lensers_update_social_link"("p_id" "uuid", "p_platform" "lensers"."lenser_social_platform", "p_url" "text", "p_label" "text") TO "service_role";



GRANT ALL ON FUNCTION "public"."fn_lensers_update_theme"("p_theme" "text") TO "anon";
GRANT ALL ON FUNCTION "public"."fn_lensers_update_theme"("p_theme" "text") TO "authenticated";
GRANT ALL ON FUNCTION "public"."fn_lensers_update_theme"("p_theme" "text") TO "service_role";
...
GRANT ALL ON FUNCTION "public"."fn_ops_create_contact"("p_name" "text", "p_email" "text", "p_subject" "text", "p_message" "text", "p_kvkk_approved" boolean, "p_ip_address" "inet", "p_user_agent" "text") TO "anon";
GRANT ALL ON FUNCTION "public"."fn_ops_create_contact"("p_name" "text", "p_email" "text", "p_subject" "text", "p_message" "text", "p_kvkk_approved" boolean, "p_ip_address" "inet", "p_user_agent" "text") TO "authenticated";
GRANT ALL ON FUNCTION "public"."fn_ops_create_contact"("p_name" "text", "p_email" "text", "p_subject" "text", "p_message" "text", "p_kvkk_approved" boolean, "p_ip_address" "inet", "p_user_agent" "text") TO "service_role";



GRANT ALL ON FUNCTION "public"."fn_public_submit_feedback"("p_product_tag" "text", "p_page" "text", "p_message" "text", "p_start_date" timestamp with time zone, "p_end_date" timestamp with time zone) TO "anon";
GRANT ALL ON FUNCTION "public"."fn_public_submit_feedback"("p_product_tag" "text", "p_page" "text", "p_message" "text", "p_start_date" timestamp with time zone, "p_end_date" timestamp with time zone) TO "authenticated";
GRANT ALL ON FUNCTION "public"."fn_public_submit_feedback"("p_product_tag" "text", "p_page" "text", "p_message" "text", "p_start_date" timestamp with time zone, "p_end_date" timestamp with time zone) TO "service_role";
...
drop policy "reactions_block_all" on "content"."reactions";

set check_function_bodies = off;

CREATE OR REPLACE FUNCTION xp.apply(p_lenser_id uuid, p_rule_key text, p_source xp.source_enum, p_source_ref_type text, p_source_ref_id uuid, p_app_id uuid DEFAULT '00000000-0000-0000-0000-000000000000'::uuid)
 RETURNS void
 LANGUAGE plpgsql
 SECURITY DEFINER
AS $function$DECLARE
...
with check (true);
  ...
  create policy "reactions_block_all"
  on "content"."reactions"
  as permissive
  for all
  to anon, authenticated
using (false)
...
CREATE TRIGGER trg_log_lenser_activity AFTER UPDATE OF last_sign_in_at ON auth.users FOR EACH ROW EXECUTE FUNCTION lensers.trg_log_lenser_activity();

CREATE TRIGGER trg_update_lenser_last_login AFTER UPDATE OF last_sign_in_at ON auth.users FOR EACH ROW EXECUTE FUNCTION lensers.trg_update_lenser_last_login();
