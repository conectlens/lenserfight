...
BEGIN
  ...
  FOR rec IN
    ...
    FROM (
      SELECT voter_id, contender_id, COUNT(*) AS cnt
        FROM battles.votes
       WHERE battle_id = p_battle_id
       GROUP BY voter_id, contender_id
    ) v
    GROUP BY v.voter_id
  ...
  FOR rec IN
    SELECT DISTINCT v1.voter_id
      FROM battles.votes v1
      JOIN battles.votes v2
        ON v2.voter_id   = v1.voter_id
...
AS $$
BEGIN
  INSERT INTO audit.vote_anomalies (battle_id, voter_id, anomaly_type, score)
...
AS $$
BEGIN
  PERFORM public.fn_check_vote_velocity(NEW.voter_id);
  RETURN NEW;
END;
$$;
