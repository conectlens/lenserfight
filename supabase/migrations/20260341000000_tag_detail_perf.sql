...
CREATE POLICY "profiles_select" ON "lensers"."profiles" FOR SELECT USING (((("visibility" = 'public'::"lensers"."lenser_visibility") AND ("status" = 'active'::"lensers"."lenser_status") AND ("deletion_requested_at" IS NULL)) OR ("user_id" = ( SELECT "auth"."uid"() AS "uid"))));
