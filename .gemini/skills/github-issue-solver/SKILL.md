...
.gemini/skills/         — available agent skills
```
...
4. Confirm whether backend validation is missing.
5. Identify affected tests.

State the root cause explicitly before designing.
...
- Information Expert: assign responsibility to the module with the required data
...
If no adequate tests exist, add them. Do not ship without coverage for the root cause.
...
- `Closes #N` only when PR fully resolves the issue
- Never use `--no-verify`
...
Load [references/pr-template.md](./references/pr-template.md) for full template.
...
If post-merge validation fails, open a follow-up issue immediately.

### Step 15 — Close the Issue

Only after merge confirmed and post-merge validation passes:
...
| Failure | Response |
|---------|----------|
| Issue not found | Ask user to confirm issue number |
| Cannot reproduce | State what was tried, ask for steps |
| Root cause unclear | Document exploration, ask for clarification |
| Tests fail after fix | Diagnose, do not suppress |
| Migration unsafe | Invoke `migration-risk-reviewer`, halt |
| RLS regression | Invoke `supabase-rls-security-reviewer`, halt |
| CI fails | Fix before merging |
| Merge conflict | Resolve, verify tests still pass |
