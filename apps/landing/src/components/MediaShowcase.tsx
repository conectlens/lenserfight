    ...
    >
      ...
      >
        ...
        <motion.div
          ref={trackRef}
          style={{ x }}
          className="flex items-center gap-5 px-5 sm:gap-8 sm:px-10 lg:gap-12 lg:px-24"
        >
...
const MediaShowcaseCard: React.FC<MediaShowcaseCardProps> = ({
  ...
  return (
    <div
      className={[
        'group relative w-[82vw] shrink-0 overflow-hidden rounded-[1.75rem] shadow-2xl',
        'transition-all duration-500 hover:ring-1 hover:ring-primary-yellow-500/20',
        'sm:rounded-[2rem] lg:w-[min(75vw,72rem)] lg:rounded-[2.5rem]',
    ...
    >
      <div className="w-full overflow-hidden">
        {item.kind === 'video' ? (
          <video
            ...
            playsInline
            preload="metadata"
            className="block h-auto w-full object-cover transition-transform duration-[2s] ease-out group-hover:scale-105"
          />
        ) : (
          <img
            src={item.src}
      ...
      {isLight ? (
        <>
          <div className="absolute inset-0 bg-gradient-to-t from-greyscale-100 via-greyscale-100/30 to-transparent opacity-90 transition-opacity duration-500 group-hover:opacity-80 pointer-events-none" />
          <div className="absolute inset-0 bg-gradient-to-r from-greyscale-100/50 via-transparent to-transparent opacity-40 pointer-events-none" />
        </>
      ) : (
        <>
          <div className="absolute inset-0 bg-gradient-to-t from-greyscale-950 via-greyscale-950/40 to-transparent opacity-90 transition-opacity duration-500 group-hover:opacity-80 pointer-events-none" />
          <div className="absolute inset-0 bg-gradient-to-r from-greyscale-950/60 via-transparent to-transparent opacity-40 pointer-events-none" />
        </>
      )}

      <div className="absolute bottom-0 left-0 right-0 z-10 w-full p-8 sm:p-12 lg:p-16 pointer-events-none">
