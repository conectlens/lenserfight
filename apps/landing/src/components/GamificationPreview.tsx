...
const LANDING_APP_ID = XP_APP_IDS.arena
...
export function GamificationPreview() {
  ...
  const { data: xpSummary } = useQuery<XPSummary | null>({
    queryKey: ['xp', 'summary', user?.id ?? 'anon', LANDING_APP_ID],
    queryFn: () => (user?.id ? xpService.getStats(user.id, LANDING_APP_ID) : null),
  ...
  const { data: seasonData } = useQuery<{ list: SeasonLeaderboardEntry[]; userEntry?: SeasonLeaderboardEntry | null }>({
    queryKey: ['xp', 'season-leaderboard', LANDING_APP_ID],
    queryFn: () => xpService.getSeasonLeaderboard(LANDING_APP_ID, undefined, 3, 0),
    staleTime: 60_000,
  })

  const { data: activeSeason } = useQuery<XPSeason | null>({
    queryKey: ['xp', 'active-season', LANDING_APP_ID],
    queryFn: () => xpService.getActiveSeason(LANDING_APP_ID),
    staleTime: 300_000,
  })
