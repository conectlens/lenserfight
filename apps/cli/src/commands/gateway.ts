...
const devices = defineCommand({
  ...
  args: {
    trust: {
      type: 'string',
      default: '',
      description: 'Filter by trust level (pending|approved|trusted|revoked)',
    },
    json: { type: 'boolean', default: false, description: 'Output JSON' },
  },
  async run({ args }) {
...
const runners = defineCommand({
  ...
  async run({ args }) {
    try {
      ...
      >('fn_runner_list_with_devices', { p_limit: 50 }, { requireAuth: true })
  ...
  async run({ args }) {
    try {
      const [allDevices, bindings] = await Promise.all([
        ...
        callRpc<Array<{ binding_status: string }>>(
...
const serve = defineCommand({
  ...
  args: {
    bind: {
      type: 'string',
      default: '127.0.0.1',
      description: 'Bind interface (127.0.0.1, 0.0.0.0, or a specific Tailscale/LAN IP)',
    },
    port: { type: 'string', default: '38080', description: 'Bind port' },
    tailscale: {
      type: 'boolean',
      default: false,
      description: 'Also bind detected Tailscale interface (requires consent file)',
    },
    'keys-only': {
      type: 'boolean',
      default: false,
      description:
        'Local-keys mode: only /healthz + /keys/*. Skips identity/session/lenser/kill_switch preconditions and sync loops. Required if you want to bind 0.0.0.0 without the Tailscale consent dance.',
    },
  },
  async run({ args }) {
      ...
      try {
        if (check === 'clock') {
          ...
          await callRpc<unknown>(
            'fn_ai_catalog_models',
            { p_provider_key: null, p_support_level: null, p_capability: null, p_modality: null },
            { noAuth: true }
          ).catch(() => null)
          ...
          results.push({
            id: 'clock',
            status: 'pass',
            message: `local clock now=${new Date(now).toISOString()} (round-trip ${elapsedMs}ms)`,
          })
          ...
          if (got !== 'ok') {
            results.push({
              id: 'keychain',
              status: 'fail',
              message: 'keychain round-trip mismatch',
            })
          } else {
          ...
          if (!secret) {
            results.push({
              id: 'identity',
              status: 'fail',
              message: 'no Ed25519 key — run `lf gateway identity rotate`',
            })
          } else {
            results.push({
              id: 'identity',
              status: 'pass',
              message: 'Ed25519 key present in keychain',
            })
          }
          ...
          if (binary) {
            results.push({
              id: 'daemon',
              status: 'pass',
              message: `daemon binary present: ${binary}`,
            })
          } else {
          ...
          try {
            ...
            if (rows && rows.length) {
              ...
              results.push({
                id: 'policy',
                status: row.global_kill_switch ? 'fail' : 'pass',
                message: `global_kill_switch=${row.global_kill_switch}`,
              })
            } else {
              results.push({
                id: 'policy',
                status: 'skipped',
                message: 'no workspace policy snapshot RPC available',
              })
            }
          } catch {
            results.push({
              id: 'policy',
              status: 'skipped',
              message: 'policy snapshot unavailable',
            })
          }
...
const identity = defineCommand({
  ...
  subCommands: {
    show: defineCommand({
      ...
      async run({ args }) {
        try {
          ...
          const meta = existsSync(identityFile)
            ? JSON.parse(readFileSync(identityFile, 'utf-8'))
            : null
          ...
          if (args.json) {
            printJson({
              ...
              signing_algo: meta?.signing_algo ?? null,
          ...
          if (!present) {
            consola.warn(
              'No Ed25519 keypair present. Run `lf gateway identity rotate` to generate one.'
            )
            return
          }
    ...
    rotate: defineCommand({
      meta: {
        name: 'rotate',
        description: 'Generate a fresh keypair (revokes the previous one locally).',
      },
      async run() {
        try {
    ...
    init: defineCommand({
      meta: {
        name: 'init',
        description: 'Generate an Ed25519 keypair for this device (alias for `identity rotate`).',
      },
      async run() {
        try {
          const { publicKey, privateKey } = generateEd25519Keypair()
...
const sync = defineCommand({
  ...
  subCommands: {
    status: defineCommand({
      ...
      async run({ args }) {
        try {
          ...
          >('fn_sync_status', { p_device_id: args.device }, { requireAuth: true })
          if (args.json) {
            printJson(rows)
            return
          }
          if (!rows?.length) {
            consola.info('No sync watermarks recorded yet for this device.')
            return
          }
          printTable(
    ...
    push: defineCommand({
      ...
      async run() {
        consola.info(
          'Outbox flush is owned by the daemon; this stub will be wired to a signed push helper in a follow-up release.'
        )
      },
    }),
  },
  ...
  subCommands: {
    show: defineCommand({
      ...
      args: {
        handle: {
          type: 'string',
          description: 'Workspace handle (e.g. @alice). Defaults to active workspace.',
        },
        json: { type: 'boolean', default: false, description: 'Output JSON' },
      },
      async run({ args }) {
        try {
          ...
          if (!settingsRows?.length) {
            consola.info(
              'No workspace policy snapshot available. Try `lf workspace settings` instead.'
            )
            return
          }
          ...
          printTable(
            ['Setting', 'Value'],
            Object.entries(snap).map(([k, v]) => [k, String(v)])
          )
        } catch (err) {
          handleError(err)
        }
    ...
    test: defineCommand({
      meta: {
        name: 'test',
        description: 'Issue a noop call to confirm the policy gate is enforced.',
      },
      args: {
        kind: {
          type: 'string',
          description: 'kill-switch | lenser-paused',
          default: 'kill-switch',
        },
      },
      async run({ args }) {
        consola.info(
          'Policy gate test (%s): server-side enforcement runs at fn_evaluate_pre_run_policy.',
          args.kind
        )
      },
    }),
...
// ---------------------------------------------------------------------------
const TAILSCALE_CONSENT_PATH = path.join(
  homedir(),
  '.lenserfight',
  'gateway',
  ...
  subCommands: {
    ...
    grant: defineCommand({
      ...
      args: {
        kind: {
          type: 'positional',
          description: 'tailscale (only supported value in v1)',
          required: true,
        },
      ...
      async run({ args }) {
        ...
        writeFileSync(TAILSCALE_CONSENT_PATH, JSON.stringify(record, null, 2), { mode: 0o600 })
...
const pair = defineCommand({
  meta: {
    name: 'pair',
    description: 'Print the bearer token apps/web uses to call the gateway. --rotate replaces it.',
  },
  args: {
    web: {
      type: 'boolean',
      description: 'Print pairing instructions for the web app',
      default: true,
    },
    rotate: {
      type: 'boolean',
      description: 'Rotate the token (invalidates the old one)',
      default: false,
    },
  },
  async run({ args }) {
