...
const versionList = defineCommand({
  meta: {
    name: 'list',
    description: 'List all versions for a prompt.',
  },
  args: {
    id: {
      type: 'positional',
  ...
  async run({ args }) {
    try {
      const versions = await callRpc<Array<Record<string, unknown>>>(
        'fn_content_list_prompt_versions',
        { p_prompt_id: args.id },
        { requireAuth: true }
      );

      if (!Array.isArray(versions) || versions.length === 0) {
        consola.info('No versions found for prompt %s.', args.id);
        return;
      }
...
const versionCreate = defineCommand({
  meta: {
    name: 'create',
    description: 'Create a new draft version for a prompt.',
  },
  args: {
    id: {
      type: 'positional',
  ...
  async run({ args }) {
    try {
      const { data: version } = await callRpc<{ data: Record<string, unknown> }>(
        'fn_content_create_prompt_version',
        {
          p_prompt_id: args.id,
      ...
      if (version?.version_number) {
        ...
        consola.info('\nTo publish: lenserfight prompt version publish %s', version.id);
      }
    } catch (err: unknown) {
      // RPC may not exist in current DB — guide the user
      const msg = (err as Error).message ?? '';
      if (msg.includes('not found') || msg.includes('does not exist')) {
        consola.error('RPC fn_content_create_prompt_version not found. Apply migration 46 first.');
...
const versionPublish = defineCommand({
  meta: {
    name: 'publish',
    description: 'Publish a draft prompt version.',
  },
  args: {
    id: {
      type: 'positional',
  ...
  async run({ args }) {
    try {
      await callRpc(
        'fn_content_publish_prompt_version',
...
// ---------------------------------------------------------------------------
// prompt resource attach
// ---------------------------------------------------------------------------
const resourceAttach = defineCommand({
  meta: {
    name: 'attach',
    description: 'Attach a local file as a resource to a prompt version slot.',
  },
  args: {
    'version-id': {
      type: 'string',
...
// ---------------------------------------------------------------------------
// prompt resource (parent)
// ---------------------------------------------------------------------------
const resource = defineCommand({
  meta: {
    name: 'resource',
    description: 'Manage prompt version resources.',
  },
  subCommands: {
...
export default defineCommand({
  meta: {
    name: 'prompt',
    description: 'Manage prompts: versions, resources.',
  },
  subCommands: {
    version,
    resource,
