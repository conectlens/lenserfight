...
import { getExecContext, handleError } from '@lenserfight/cli-client'
...
export default defineCommand({
  ...
  async run({ args }) {
    ...
    // The TUI REPL runs stdin in raw mode and never lets Ctrl+C reach the OS
    // as a real SIGINT — it intercepts the keypress and cancels via this
    // shared signal instead, so it must be honored here too.
    const replSignal = getExecContext().cancelSignal
    if (replSignal) {
      if (replSignal.aborted) ac.abort()
      else replSignal.addEventListener('abort', () => ac.abort(), { once: true })
    }
