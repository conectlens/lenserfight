    ...
    buildPaths(env, _p) {
      const paths: LabyrinthPath[] = [
        ...
        },
        {
          ...
          sublabel: '<SLUG-or-ID>',
          action: { type: 'prompt', prefill: 'battle stream ' },
          unlocked: true,
        },
        {
          id: 'leaderboard',
      ...
      if (env.ollama) {
        paths.push({
          ...
          action: { type: 'prompt', prefill: 'battle create --local --lenser-a  --lenser-b ' },
          unlocked: true,
          badge: 'LOCAL',
        })
      }
    ...
    buildPaths(env, _p) {
      return [
        ...
        },
        {
          id: 'local_schedule',
          key: 's',
          label: 'Schedule Local Run',
          sublabel: '--workflow <ID> --lenser <ID>',
          action: { type: 'prompt', prefill: 'schedule create --local --workflow  --lenser ' },
          unlocked: env.workflowCount > 0,
        },
      ]
    },
    recommend(_env, _p) {
