...
export const LenserActionsList: React.FC<LenserActionsListProps> = ({ actions }) => {
  ...
  const getIcon = (reaction: string) => {
    switch (reaction) {
      ...
      case 'clap':
        return <PartyPopper size={18} className="text-yellow-500" />
      default:
          ...
          <div className="flex-1 min-w-0">
            <div className="flex justify-between items-start mb-0.5">
              <p className="text-sm text-gray-500 dark:text-gray-400 font-medium">
                {getLabel(action.reaction, action.targetType)}
              </p>
              <span className="text-xs text-gray-400 dark:text-gray-500 whitespace-nowrap ml-2">
                {timeAgo(action.createdAt)}
              </span>
            </div>
            <p className="text-base font-bold text-gray-900 dark:text-white line-clamp-1 break-words">
              {action.targetTitle}
            </p>
          </div>
        </Card>
