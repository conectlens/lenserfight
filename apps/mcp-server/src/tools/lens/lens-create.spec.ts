...
describe('create_lens tool', () => {
  ...
  it('normalizes params with optional defaulting to false', async () => {
    ...
    expect(lensService.create).toHaveBeenCalledWith(
      expect.anything(),
      expect.objectContaining({
        params: [
          { label: 'A', optional: false },
  ...
  it('teaches MCP clients how Lens parameters feed workflow creation', () => {
    const tool = captureTool(registerLensCreate)
    expect(tool.description).toContain('[[Parameter Label]]')
    expect(tool.description).toContain('[[Context:textarea]]')
