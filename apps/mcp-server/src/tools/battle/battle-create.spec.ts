...
describe('create_battle tool', () => {
  ...
  it('creates battle and applies config when supplied', async () => {
    ...
    const env = parseEnvelope(await tool.handler({
      title: 'My Battle', task_prompt: 'Write a poem',
      battle_type: 'ai_vs_ai', judging_mode: 'ai_judge', max_contenders: 4,
    }));
    expect(env.success).toBe(true);
    const data = env.data as { id: string; title: string };
    expect(data).toEqual({ id: 'b1', title: 'My Battle' });
    expect(battleService.updateConfig).toHaveBeenCalledWith(expect.anything(), expect.objectContaining({
      battle_id: 'b1', battle_type: 'ai_vs_ai', judging_mode: 'ai_judge', max_contenders: 4,
    }));
  });

  it('skips updateConfig when no config fields supplied', async () => {
    (battleService.create as jest.Mock).mockResolvedValue('b1');
    const tool = captureTool(registerBattleCreate);
    await tool.handler({ title: 'My Battle', task_prompt: 'p' });
    expect(battleService.updateConfig).not.toHaveBeenCalled();
  });
