...
const meta = getToolMeta('list_agent_tool_catalog');
const TOOL = meta.name;

export function registerAgentListToolCatalog(
  server: McpServer,
  sb: SupabaseClient,
  lenserId?: string
): void {
  registerMcpTool(
    ...
    async (args) => {
      ...
      try {
        const { items, total } = await agentService.listToolCatalog(sb, { owner_lenser_id: owner });
