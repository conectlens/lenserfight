...
export async function handlePartnersModelsRoute(
  req: IncomingMessage,
  res: ServerResponse,
  partnerName: string,
  requestId: string,
  ...
  const models = await service.getAiModels(provider, auth.user.id)
  sendSuccess(res, 200, models, requestId, startedAt)
}
