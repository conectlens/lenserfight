...
export async function handlePartnersProvisionRoute(
  req: IncomingMessage,
  res: ServerResponse,
  partnerName: string,
  requestId: string,
  ...
  const record = await service.provision(provider, auth.user)
  sendSuccess(res, 200, record, requestId, startedAt)
}
