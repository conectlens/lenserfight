...
export async function handlePartnersBalanceRoute(
  req: IncomingMessage,
  res: ServerResponse,
  partnerName: string,
  requestId: string,
  ...
  const balance = await service.getBalance(provider, auth.user.id)
  sendSuccess(res, 200, balance, requestId, startedAt)
}
