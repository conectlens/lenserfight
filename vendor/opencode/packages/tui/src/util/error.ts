...
export function cliErrorMessage(input: unknown): string | undefined {
  ...
  if (model) {
    ...
    return [
      ...
      "Try: `lf models` to list available models",
      "Or check your config (lenserfight.json) provider/model names",
    ].join("\n")
  }
  ...
  if (isRecord(input) && named(input, "MCPFailed")) {
    ...
    return `MCP server "${name}" failed. Note, the assist runtime does not support MCP authentication yet.`
  }
  return undefined
}
