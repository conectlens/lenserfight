...
function globalConfigFile() {
  const candidates = ["lenserfight.jsonc", "lenserfight.json", "config.json"].map((file) =>
...
const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const loadGlobal = Effect.fnUntraced(function* (env?: Record<string, string>) {
      ...
      result = mergeConfig(result, yield* loadFile(path.join(Global.Path.config, "lenserfight.json"), env))
      result = mergeConfig(result, yield* loadFile(path.join(Global.Path.config, "lenserfight.jsonc"), env))
    ...
    const loadInstanceState = Effect.fn("Config.loadInstanceState")(
      function* (ctx: InstanceContext) {
        ...
        for (const dir of directories) {
          if (dir.endsWith(".lenserfight") || dir === Flag.OPENCODE_CONFIG_DIR) {
            for (const file of ["lenserfight.json", "lenserfight.jsonc"]) {
          ...
          // Auto-discovered plugins under `.lenserfight/plugin(s)` are already local files, so ConfigPlugin.load
