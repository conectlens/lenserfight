...
export const McpAuthCommand = effectCmd({
  ...
  handler: Effect.fn("Cli.mcp.auth")(function* (args) {
    ...
    if (servers.length === 0) {
      ...
      prompts.log.info("Remote MCP servers support OAuth by default. Add a remote server in lenserfight.json:")
      prompts.log.info(`
  "mcp": {
    "my-server": {
      "type": "remote",
...
async function resolveConfigPath(baseDir: string, global = false) {
  // Check for existing config files (prefer .jsonc over .json, check .lenserfight/ subdirectory too)
  const candidates = [path.join(baseDir, "lenserfight.json"), path.join(baseDir, "lenserfight.jsonc")]

  if (!global) {
    candidates.push(
      path.join(baseDir, ".lenserfight", "lenserfight.json"),
      path.join(baseDir, ".lenserfight", "lenserfight.jsonc"),
    )
  }
  ...
  // Default to lenserfight.json if none exist
  return candidates[0]
}
