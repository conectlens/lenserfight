...
|--------|--------------|-------|
| Proje | `.lenserfight/config.json` | Mod, port, uygulama listesi — commit edilebilir |
...
| Eski cihaz | `~/.lenserfight/config.json` | Geriye dönük uyumluluk; yazıda yansıtılır |
...
3. Eski `~/.lenserfight/config.json` (yalnızca yedek okuma)
...
```json
{
  "workspaces": {
    "/home/kullanici/projeler/lenserfight-projem": {
      ...
      "configPath": "/home/kullanici/projeler/lenserfight-projem/.lenserfight/config.json"
    }
  }
}
```
