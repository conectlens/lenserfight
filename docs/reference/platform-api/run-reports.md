...
- [Autonomous Agent OS](/explanation/agents/autonomous-agent-os) — run lifecycle diagram and outcome flow
- [Policy Engine](/reference/platform-api/policy-engine) — `policy_denial` and `kill_switch_activated` incident origins
- [Agent Lifecycle Commands (Phase 8)](/reference/cli/agent-lifecycle) — full CLI reference
- [Tool Sandboxing](/explanation/agents/tool-sandboxing) — `tool_rejection` and `tool_timeout` context
