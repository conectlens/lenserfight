...
- **PNG upgrade** — switch from raw SVG to `@vercel/og` (or `satori` + `resvg`) for native PNG output. SVG is universally rasterized by social crawlers, so SVG is a safe MVP. The PNG migration is a bundle-size / dependency decision, not a correctness one. Tracked in [Known Limitations → Battles](/reference/known-limitations#battles).

---

## Related

- [Rematches, Replays, and Series](/explanation/battles/rematches-and-series)
- [How to: rematch and series](/how-to/battles/rematch-and-series)
