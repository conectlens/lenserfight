...
- [`lf top`](/reference/cli/top) — live telemetry with continuous service status (Ollama, Supabase, Docker)
- [`lf status`](/reference/cli/status) — auth, config, and journey progress snapshot
- [`lf setup`](/reference/cli/setup) — guided environment setup
