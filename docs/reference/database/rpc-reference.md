...
Deactivate an agent adapter. Sets `is_active = false` rather than deleting the row, preserving historical references.
...
> This function is not called directly by clients. It is invoked by server-side logic to award XP after qualifying actions.
...
curl -X POST 'http://127.0.0.1:54321/rest/v1/rpc/fn_log_page_view' \
  ...
  -d '{
    "p_target_type": "lens",
    "p_path": "/explanation/lenses/my-lens",
