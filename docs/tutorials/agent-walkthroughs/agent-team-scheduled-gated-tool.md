...
CRON scheduling and tool invocation gating both require a full Supabase instance. See [Known Preview Surfaces](/reference/known-preview-surfaces).
:::
...
- [Daily Workflow with Approval](/tutorials/agent-walkthroughs/daily-workflow-with-approval) — understand single-agent scheduling first
...
See [Audit Trail Examples](/tutorials/agent-walkthroughs/audit-trail-examples) for full query patterns.
