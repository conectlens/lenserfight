...
- **K4 — `/health` probe.** `public.fn_health()` returns `ok` and the platform-api `/health` route reports green. Verified by `pnpm announcement:dashboard --once`. See [Known Preview Surfaces](/en/reference/known-preview-surfaces#feature-surface-table).
...
1. Read the [Limited Beta status runbook](/en/explanation/battles/limited-beta-status) and the [Battle Integrity Checklist](/en/how-to/battles/battle-integrity-checklist).
