...
| `.env.example`, `.env.mcp.local.example` | Environment configuration | Keep | Root is the only location `libs/utils/env/src/lib/runtimeConfig.ts` and the installation docs point to; `.env.mcp.local.example` pairs with root `.mcp.json` and has no overlapping content with `.env.example`. |
| `.env.development.example` | Environment configuration | **Corrected: Removed** | This audit originally called this "not duplicated" — wrong. It shared real content with `.env.example` (Supabase creds, Chainabit config) with only placeholder text differing, and its `PLATFORM_API_PORT`/`PLATFORM_API_CORS_ORIGIN` values (correct, matching `libs/utils/env/src/lib/env.ts`) revealed that `.env.example`'s own bare `PORT`/`CORS_ORIGIN` entries were actually wrong — those two vars don't exist unprefixed anywhere in the codebase. Consolidated: `.env.example` now carries the corrected var names plus everything `.env.development.example` had that it was missing (`ECHO_PROVIDER`, additional AI provider key placeholders, `ENV_MODE`, local-Chainabit-instance setup as an alternative to the production Chainabit section). `scripts/dev-start.sh` now bootstraps `.env.development.local` from `.env.example` instead. |
