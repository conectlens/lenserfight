...
5. Read the failure-rate badges under the workflow chart. Use them to decide which workflow should be opened in [Reports](./reports.md), [Runs](./runs.md), or [Logs](./logs.md).

## Interpreting changes

| Signal | What to check next |
|---|---|
| Cost rises and run count is flat | Review [Models](./models.md), [Providers](./providers.md), and [Cost](./cost.md). A model or provider may have changed. |
| Cost rises and run count rises | Review [Schedules](./schedules.md) and [Workflows](./workflows.md). Automation volume may be healthy but newly frequent. |
| Pass rate drops after an edit | Reopen [Instructions](./instructions.md), [Personality](./personality.md), or the workflow assignment that changed. |
| p95 duration grows while p50 stays stable | Inspect [Tool Invocation](./drawers/tool-invocation.md) and provider reachability. Tail latency is usually external dependency pressure. |
| Failure rate grows for one workflow | Open [Run Detail](./drawers/run-detail.md), then compare the failed report and log events. |
...
5. Check [Cost](./cost.md) to make sure quota and budget rules still fit the new usage.
...
- [Creator Analytics Section](./creator-analytics.md)
