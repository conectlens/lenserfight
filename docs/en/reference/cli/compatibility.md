...
Color output and the [interactive shell](/en/how-to/operations/cli-dashboard) are enabled only when `process.stdout.isTTY` is `true`. When stdout is piped or redirected (e.g., `lf lenses | grep foo`), all ANSI sequences are stripped automatically and bare `lf` renders a single static frame instead of the interactive shell.
