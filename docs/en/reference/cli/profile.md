...
See also: [`lf completion`](./completion.md) for shell completion of subcommands.
...
- Profile files are written with mode `0600`. Anyone with read access to the operator's home directory can still read them — see [Known Limitations](/en/reference/known-limitations#api-and-docs).
