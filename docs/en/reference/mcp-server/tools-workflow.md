...
description: Reference for all 8 workflow tools in the LenserFight MCP server — list, get, create, run, poll status, read logs, retry, and summarize workflow runs.
---
...
Use `workflow_run_status` to poll the current status of a run. Use `workflow_retry` to start a new run when a previous one failed or was cancelled.
...
Start a workflow execution. Returns a `run_id` immediately; use `workflow_run_status` to poll for completion.
...
| `failed` | One or more nodes failed; use `workflow_run_logs` for details |
