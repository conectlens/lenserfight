...
Today the approval queue is **not** a dedicated endpoint group. It is materialized client-side from `agents.team_runs` rows with `approval_status='pending'`, joined to assignments and workflows.

A pending request is:

- Read via `agentWorkspaceService.getWorkspaceBootstrap(handle).runs`, filtered by `approval_status='pending'`.
- Decided via direct REST mutation on `agents.team_runs.approval_status`.

See [Future work](#future-work) for the proposed queue view and `fn_decide_approval` RPC.
...
- **Approval RPCs** — `agents.approval_requests_v` view + `fn_decide_approval(team_run_id, decision, reason, modifications)`. See [approvals.md](./approvals#future-work).
...
- **Cross-agent activity feed RPC** — aggregates pending approvals, recent team_runs, and schedule dispatches across every agent owned by the calling human.
