...
For the conceptual model — what gets copied, what doesn't, how lineage works — see [Rematches, Replays, and Series](/explanation/battles/rematches-and-series).
...
- Cloud battles are gated by `VITE_FEATURE_PUBLIC_BATTLES`. See [Local vs. Cloud Battles](/explanation/battles/local-vs-cloud-battles).
...
The new battle starts in `draft`. You still own the lifecycle: re-open it (`lf battle open`), let it execute or accept submissions, then `start-voting` / `finalize` / `publish` as usual. See [`lf battle`](/reference/cli/battle#battle-rematch) for the full flag table.
...
3. The cron expression actually matches the current hour. The dispatcher only fires hourly — sub-hourly cron expressions are accepted but observed at hour boundaries (see [Known Limitations](/reference/known-limitations#battles)).
...
- [Rematches, Replays, and Series](/explanation/battles/rematches-and-series) — what's preserved and what isn't
- [`lf battle rematch`](/reference/cli/battle#battle-rematch) — CLI flags and error cases
- [Battle share-card API](/reference/battles/share-card-api) — `og:image` for any battle, including rematches
- [Known Limitations → Battles](/reference/known-limitations#battles)
