...
- [Autonomous Agent OS](/explanation/agents/autonomous-agent-os) — dark launch in the governance controls table
- [Policy Engine](/reference/platform-api/policy-engine) — `dark_launch` policy type and `dark_launch_excluded` reason
- [Agent Lifecycle Commands (Phase 8)](/reference/cli/agent-lifecycle) — full `lf dark-launch` CLI reference
- [Using the Kill Switch](/how-to/kill-switch) — emergency stop if a dark-launched workflow misbehaves
