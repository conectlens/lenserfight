...
Failures are recorded in `event_dispatches.status='failed'` with the SQL error message. Operators see them via [`lf automation history`](/reference/cli/automation-rules) or the `/automations` web page.
...
- [Build your first trigger](/how-to/automation/build-your-first-trigger) — concrete walkthrough
- [Trigger rule schema](/reference/automation/trigger-rule-schema) — full YAML reference
- [`lf automation` CLI](/reference/cli/automation-rules) — command reference
- [Webhook outbox (Phase P3)](/explanation/community/task-schema-governance) — signing, retries, dead-lettering
