...
Single-node workflow that produces an image via `fal-ai/flux/dev` and lands a
...
- `WORKFLOW.md` — declarative workflow definition (frontmatter + DAG).
- `seed.sql` — idempotent seed that inserts the workflow as the seeded Alice
...
- `execution.fn_run_lens` (or workflow runner) → creates `execution.runs`.
