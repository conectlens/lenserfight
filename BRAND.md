# LenserFight brand and trademark guidelines

The **LenserFight** name, logos, and related marks identify the project and the official product experience at [lenserfight.com](https://lenserfight.com) and [docs.lenserfight.com](https://docs.lenserfight.com).

The **source code** in this repository is licensed under the [Apache License 2.0](LICENSE). That license governs copyright and patents for the software. It does **not** grant permission to use LenserFight trademarks in ways that suggest endorsement, affiliation, or official status when none exists.

## Allowed without asking

- **Use, modify, and redistribute** the software under the terms of the Apache-2.0 license.
- **Self-host** Community Edition for development, evaluation, research, or internal use.
- **Contribute** via pull requests and participate in public discussions.
- **Say truthfully** that your product or deployment is built using or derived from LenserFight source code.
- **Use “Powered by LenserFight”** (or similar factual attribution) when your offering materially includes this codebase, provided you do not imply that LenserFight maintains or endorses your specific deployment.

## Ask first or avoid

- Using the **LenserFight wordmark or logo** as your product or company identity, or in a way that could confuse users about who operates the service.
- Implying **official partnership**, **sponsorship**, or **maintainer endorsement** without written agreement.
- Registering domains, social handles, or package names that are **confusingly similar** to LenserFight for the purpose of impersonation or passing off.

## Official channels

Canonical project information, downloads we endorse, and the hosted product are associated with **lenserfight.com** and this organization’s official repositories and release notes.

## Chainabit

**Chainabit** is a separate brand and product. It is the private commercial API platform that powers the cloud product at [lenserfight.com](https://lenserfight.com). Chainabit is not part of this repository and is not covered by the Apache-2.0 license in this repo.

- The **Chainabit** name and any Chainabit marks are governed separately and are not subject to LenserFight's trademark guidelines above.
- Saying that a product "uses the LenserFight connector SDK to integrate with Chainabit" is a factual technical description and is allowed.
- Do not use **Chainabit** marks in a way that implies affiliation, endorsement, or that you operate the Chainabit commercial platform.

For questions about Chainabit branding or the commercial relationship between LenserFight and Chainabit, contact maintainers through [SUPPORT.md](SUPPORT.md).

## Questions

For trademark or brand-use questions not covered here, open a discussion or contact maintainers through [SUPPORT.md](SUPPORT.md). For security vulnerabilities, use [SECURITY.md](SECURITY.md), not public issues.
